...
"""
Database connection and session management for ScrnaClaw Graph Memory.

Ported from nocturne_memory with ScrnaClaw configuration.
...
# Default DB URL — uses ~/.config/scrna_claw/memory.db
_DEFAULT_DB_DIR = Path.home() / ".config" / "scrna_claw"
_DEFAULT_DB_URL = f"sqlite+aiosqlite:///{_DEFAULT_DB_DIR / 'memory.db'}"


def _get_database_url() -> str:
    """Resolve database URL from environment or default."""
    return os.getenv("SCRNA_CLAW_MEMORY_DB_URL", _DEFAULT_DB_URL)
    ...
    async def init_db(self):
        ...
        try:
            ...
            raise RuntimeError(
                ...
                f"  Error: {e}\n\n"
                f"Troubleshooting:\n"
                f"  - Check SCRNA_CLAW_MEMORY_DB_URL in .env\n"
