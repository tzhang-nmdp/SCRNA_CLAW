"""
ScrnaClaw Memory API Server.

FastAPI application serving the graph memory REST API.
Start with: python -m scrna_claw.memory.server
         or: oc memory-server
...
def _validate_server_security(host: str, api_token: str) -> None:
    ...
    raise SystemExit(
        ...
        "SCRNA_CLAW_MEMORY_API_TOKEN. Bind to 127.0.0.1/localhost or set a bearer token."
    )
...
def _build_app():
    ...
    app = FastAPI(
        title="ScrnaClaw Memory API",
        description="Graph-based memory system for ScrnaClaw multi-omics platform",
        version="1.0.0",
        lifespan=lifespan,
    )
    ...
    _api_token = os.getenv("SCRNA_CLAW_MEMORY_API_TOKEN", "")
...
def main():
    ...
    host = os.getenv("SCRNA_CLAW_MEMORY_HOST", DEFAULT_MEMORY_API_HOST)
    port = int(os.getenv("SCRNA_CLAW_MEMORY_PORT", str(DEFAULT_MEMORY_API_PORT)))
    api_token = os.getenv("SCRNA_CLAW_MEMORY_API_TOKEN", "")
    _validate_server_security(host, api_token)

    print(f"ScrnaClaw Memory API starting on http://{host}:{port}")
    if api_token:
