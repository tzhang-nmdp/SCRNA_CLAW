...
Ported from nocturne_memory for ScrnaClaw Memory System.
...
Storage: one JSON file at `~/.config/scrna_claw/memory_snapshots/changeset.json`.
"""
...
def _default_snapshot_dir() -> str:
    env_dir = os.environ.get("SCRNA_CLAW_SNAPSHOT_DIR")
    if env_dir:
        return env_dir
    return str(Path.home() / ".config" / "scrna_claw" / "memory_snapshots")
