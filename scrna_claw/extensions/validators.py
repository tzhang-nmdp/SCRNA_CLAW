"""Validation helpers for installable ScrnaClaw extension packs."""
...
from scrna_claw.extensions.manifest import (
    ExtensionManifest,
...
def validate_extension_directory(
    ...
    *,
    ...
    if manifest_path is not None:
        ...
        warnings.append(
            "Legacy skill pack detected without scrna_claw-extension.json; installing with inferred 'skill-pack' type."
        )
    else:
        errors.append(
            "No scrna_claw-extension.json manifest found, and the directory does not match the legacy skill-pack layout."
        )
    ...
    if extension_type == "skill-pack":
        if not script_paths and not has_skill_md:
            errors.append(
                ...
                "The skill may not be loadable. Make sure the directory follows ScrnaClaw conventions."
            )
        ...
        if manifest is None:
            errors.append(
                f"Extension type '{extension_type}' requires an scrna_claw-extension.json manifest."
            )
