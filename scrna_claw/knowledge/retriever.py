"""
High-level query interface for the ScrnaClaw knowledge base.
...
_DEFAULT_KB_PATH = Path(
    os.getenv("SCRNA_CLAW_KNOWLEDGE_PATH", "")
) if os.getenv("SCRNA_CLAW_KNOWLEDGE_PATH") else None
...
def _find_kb_root() -> Path:
    ...
    # Check relative to this file (scrna_claw/knowledge/retriever.py → project root)
...
class KnowledgeAdvisor:
    ...
    def build(self, kb_path: Optional[Path] = None) -> dict:
        ...
        if not root.is_dir():
            raise FileNotFoundError(
                ...
                "Set SCRNA_CLAW_KNOWLEDGE_PATH or pass --path."
            )
    ...
    def search_with_trace(
        ...
        if not self.ensure_available(auto_build=auto_build):
            raise RuntimeError(
                "Knowledge base not built yet. Run: python scrna_claw.py knowledge build"
            )
