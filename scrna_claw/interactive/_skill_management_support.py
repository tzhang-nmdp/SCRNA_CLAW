...
from scrna_claw.interactive._session import format_relative_time
from scrna_claw.runtime.events import EVENT_EXTENSION_INSTALLED
from scrna_claw.runtime.hook_payloads import ExtensionHookPayload
from scrna_claw.runtime.hooks import (
    HOOK_MODE_NOTICE,
...
def _extension_install_hook_statuses(
    *,
    ...
    runtime = build_default_lifecycle_hook_runtime(scrna_claw_dir)
    runtime.emit(
...
def _resolve_install_dest(
    *,
    ...
    return extension_store_dir(scrna_claw_dir, extension_type) / install_name
...
def _build_post_install_statuses(
    *,
    ...
    snapshot = build_extension_runtime_snapshot(scrna_claw_dir)
    ...
    *,
    ...
    try:
        write_install_record(
            ...
            extension_type=validation.extension_type,
            relative_install_path=str(dest.relative_to(Path(scrna_claw_dir))),
        )
        write_extension_state(
            dest,
...
def install_extension_from_source(
    ...
    *,
    ...
    staging_base = Path(scrna_claw_dir) / "installed_extensions" / ".staging"
...
def prepare_extension_uninstall_plan(
    ...
    *,
    ...
    if not matches and expected_type == "skill-pack":
        skills_dir = Path(scrna_claw_dir) / "skills"
        found_builtin = any(
        ...
        if found_builtin:
            return SkillCommandStatus(
                ...
                "Built-in skills are part of the ScrnaClaw core and should not be deleted.",
            )
...
def build_installed_extension_list_view(
    *,
    ...
    runtime_snapshot = build_extension_runtime_snapshot(scrna_claw_dir)
    activation_by_path = {
...
def build_refresh_extensions_statuses(
    *,
    ...
    view = build_installed_extension_list_view(scrna_claw_dir=scrna_claw_dir)
    runtime_snapshot = build_extension_runtime_snapshot(scrna_claw_dir)
    statuses.append(
        SkillCommandStatus(
            "info",
...
def build_refresh_skills_statuses(
    *,
    ...
    view = build_installed_skill_list_view(scrna_claw_dir=scrna_claw_dir)
