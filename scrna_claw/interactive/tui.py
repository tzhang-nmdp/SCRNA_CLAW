"""ScrnaClaw Textual TUI — full-screen terminal user interface.
...
if _HAS_TEXTUAL:
    ...
    class ScrnaClawTUI(App):
        """ScrnaClaw full-screen terminal chat interface."""
        ...
        def __init__(
            ...
            self._workspace = workspace_dir or str(_SCRNA_CLAW_DIR)
            self._model = model
        ...
        def compose(self) -> ComposeResult:
            with Horizontal(id="header-bar"):
                yield Label("⬡ ScrnaClaw", id="header-title")
                yield Label(
                    build_tui_header_label(
                        model=self._model,
                        session_id=self._session_id,
        ...
        async def _init_llm_async(self) -> None:
            try:
                ...
                model = os.environ.get("SCRNA_CLAW_MODEL", self._config.get("model", ""))
...
def run_tui(
    ...
    """Launch the ScrnaClaw Textual TUI application."""
    if not _HAS_TEXTUAL:
        raise ImportError(
