...
from scrna_claw.memory.scoped_memory import normalize_scoped_memory_scope
from scrna_claw.runtime.output_styles import normalize_output_style_name
from scrna_claw.runtime.transcript_store import build_transcript_summary
...
def _load_enabled_extension_refs(scrna_claw_dir: str | Path | None) -> list[str]:
    root = _normalize_text_field(scrna_claw_dir)
    if not root:
        return []
    try:
        names = []
...
def enrich_session_metadata(
    ...
    *,
    ...
    workspace_dir: str = "",
    pipeline_workspace: str | None | object = _SESSION_METADATA_UNSET,
    scrna_claw_dir: str | Path | None = None,
) -> dict[str, Any]:
    ...
    enabled_extension_refs = _load_enabled_extension_refs(scrna_claw_dir)
...
def build_export_session_command_view(
    ...
    *,
    ...
    export_path = export_dir / f"scrna_claw_session_{session_id}.md"
    try:
