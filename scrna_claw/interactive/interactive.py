"""ScrnaClaw interactive CLI — prompt_toolkit REPL loop.

Provides a rich terminal chat interface powered by ScrnaClaw's existing
...
from scrna_claw.common.user_guidance import strip_user_guidance_lines
...
from ._scrna_claw_actions import (
...
# ScrnaClaw paths
# ---------------------------------------------------------------------------
_SCRNA_CLAW_DIR = Path(__file__).resolve().parent.parent.parent
...
def print_banner(
    ...
    ws = workspace_dir or str(_SCRNA_CLAW_DIR)
...
def _session_metadata_from_state(state: dict[str, Any]) -> dict[str, Any]:
    metadata = enrich_session_metadata(
        ...
        scrna_claw_dir=_SCRNA_CLAW_DIR,
    )
...
def _handle_doctor(
    ...
    _apply_session_command_view(
        ...
        build_doctor_command_view(
            ...
            scrna_claw_dir=str(_SCRNA_CLAW_DIR),
            output_dir=str(_SCRNA_CLAW_DIR / "output"),
        ),
    )
...
def _handle_config(arg: str) -> None:
    ...
    if subcmd == "list":
        ...
        table = Table(title="ScrnaClaw Configuration", show_header=True)
...
def _handle_install_extension(arg: str) -> None:
    for status in install_extension_from_source(arg, scrna_claw_dir=_SCRNA_CLAW_DIR):
...
def _handle_install_skill(arg: str) -> None:
    ...
    statuses = install_skill_from_source(arg, scrna_claw_dir=_SCRNA_CLAW_DIR)
...
def _handle_installed_extensions() -> None:
    view = build_installed_extension_list_view(scrna_claw_dir=_SCRNA_CLAW_DIR)
    if not view.entries:
        console.print(f"[yellow]{view.empty_text}[/yellow]")
        console.print(f"[dim]{view.hint_text}[/dim]")
        return
...
def _handle_installed_skills() -> None:
    view = build_installed_skill_list_view(scrna_claw_dir=_SCRNA_CLAW_DIR)
    if not view.entries:
        console.print(f"[yellow]{view.empty_text}[/yellow]")
        console.print(f"[dim]{view.hint_text}[/dim]")
        return
...
def _handle_refresh_extensions() -> None:
    for status in build_refresh_extensions_statuses(scrna_claw_dir=_SCRNA_CLAW_DIR):
        _print_skill_command_status(status)


def _handle_refresh_skills() -> None:
    for status in build_refresh_skills_statuses(scrna_claw_dir=_SCRNA_CLAW_DIR):
        _print_skill_command_status(status)
...
async def _run_research_pipeline(
    ...
    try:
        from scrna_claw.agents import _check_research_deps
    ...
    try:
        from scrna_claw.agents.pipeline import ResearchPipeline
        from scrna_claw.agents.pipeline_result import normalize_pipeline_result
...
def _init_llm(config: dict) -> tuple[str, str]:
    ...
    try:
        sys.path.insert(0, str(_SCRNA_CLAW_DIR))
        import bot.core as core
        load_project_dotenv(_SCRNA_CLAW_DIR, override=False)

        provider = os.environ.get("LLM_PROVIDER", config.get("provider", ""))
        api_key = os.environ.get("LLM_API_KEY", config.get("api_key", ""))
        model = os.environ.get("SCRNA_CLAW_MODEL", config.get("model", ""))
        base_url = os.environ.get("LLM_BASE_URL", config.get("base_url", ""))

        import logging
        logging.getLogger("scrna_claw.bot").setLevel(logging.WARNING)
        # Suppress verbose loggers in CLI mode to make output cleaner
        logging.getLogger("httpx").setLevel(logging.WARNING)
        logging.getLogger("httpcore").setLevel(logging.WARNING)
        logging.getLogger("scrna_claw.memory").setLevel(logging.WARNING)
        logging.getLogger("scrna_claw.memory.snapshot").setLevel(logging.WARNING)
        ...
        return core.SCRNA_CLAW_MODEL, core.LLM_PROVIDER_NAME
    except Exception as e:
        logger.warning("LLM init error: %s", e)
        return os.environ.get("SCRNA_CLAW_MODEL", "unknown"), os.environ.get("LLM_PROVIDER", "")
...
async def _async_interactive_loop(
    ...
    # flooding the terminal.  Override with SCRNA_CLAW_LOG_LEVEL=INFO.
    _cli_log_level = os.environ.get("SCRNA_CLAW_LOG_LEVEL", "WARNING").upper()
    for noisy in (
        "scrna_claw.bot", "scrna_claw.knowledge", "scrna_claw.core",
        "scrna_claw.memory", "scrna_claw.runtime", "scrna_claw.runtime.context_layers",
    ...
    effective_workspace = workspace_dir or str(_SCRNA_CLAW_DIR)
    ...
    while state["running"]:
        try:
            ...
            if command is not None and command.name == "/exit":
                ...
                table = Table(title="ScrnaClaw Commands", show_header=True, header_style="bold cyan")
...
def run_interactive(
    ...
    """Launch the interactive ScrnaClaw CLI session.
...
async def _single_shot(
    ...
    await _stream_llm_response(
        messages,
        workspace_dir=workspace_dir or str(_SCRNA_CLAW_DIR),
        pipeline_workspace="",
    )
    ...
    await save_session(
        ...
        messages,
        model=resolved_model,
        workspace=workspace_dir or str(_SCRNA_CLAW_DIR),
        metadata={},
        transcript=messages,
    )
