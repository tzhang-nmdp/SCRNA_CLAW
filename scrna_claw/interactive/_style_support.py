...
from scrna_claw.runtime.output_styles import (
...
def build_style_command_view(
    ...
    *,
    ...
    if command == "list":
        profiles = get_output_style_profiles(scrna_claw_dir)
    ...
    if command == "set":
        ...
        if requested_name not in registry:
            available = ", ".join(profile.name for profile in get_output_style_profiles(scrna_claw_dir))
            return SessionCommandView(
                output_text=(
