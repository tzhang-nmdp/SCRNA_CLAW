"""Shared ScrnaClaw command actions for interactive surfaces."""
...
def load_scrna_claw_script():
    """Load the root-level ``scrna_claw.py`` script via importlib."""
    script_path = _SCRNA_CLAW_DIR / "scrna_claw.py"
    spec = importlib.util.spec_from_file_location("_scrna_claw_script", script_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Cannot load scrna_claw.py from: {script_path}")
...
def build_skills_catalog_view(domain_filter: str | None = None) -> SkillsCatalogView:
    scrna_claw_script = load_scrna_claw_script()
    skills = getattr(scrna_claw_script, "SKILLS")
    domains = getattr(scrna_claw_script, "DOMAINS")
    workflow_order = getattr(scrna_claw_script, "_WORKFLOW_ORDER", {})
...
def format_skills_catalog_plain(view: SkillsCatalogView) -> str:
    lines = [f"ScrnaClaw Skills ({view.total_skills} total)", "=" * 40]
...
def run_skill_command(command: SkillRunCommandArgs) -> dict[str, Any]:
    scrna_claw_script = load_scrna_claw_script()
    return scrna_claw_script.run_skill(
        command.skill,
