...
# Default location for ScrnaClaw R scripts
...
class RScriptRunner:
    ...
    scripts_dir : Path, optional
        Directory containing R scripts.  Defaults to ``scrna_claw/r_scripts/``.
    timeout : int
    ...
    def _build_r_env(self) -> dict[str, str]:
        ...
        runtime that differs from the current ScrnaClaw environment.
        """
        env = os.environ.copy()
        ...
        if conda_prefix:
            r_libs_user = Path(conda_prefix) / "lib" / "R" / "scrna_claw-library"
            r_libs_user.mkdir(parents=True, exist_ok=True)
            env.setdefault("SCRNA_CLAW_R_LIBS", str(r_libs_user))
