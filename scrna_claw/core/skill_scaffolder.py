"""Helpers for creating ScrnaClaw-native skill scaffolds."""
...
import scrna_claw
from scrna_claw.common.manifest import StepRecord
from scrna_claw.runtime.hooks import build_default_lifecycle_hook_runtime
from scrna_claw.runtime.verification import (
...
SKILL_TEMPLATE_PATH = SCRNA_CLAW_DIR / "templates" / "SKILL-TEMPLATE.md"
STAGING_ROOT = SCRNA_CLAW_DIR / ".scrna_claw-staging" / "skill-scaffolds"
...
def render_skill_markdown(
    *,
    ...
    profile = _DOMAIN_PROFILES[domain]
    summary = (summary or "").strip() or f"Autogenerated ScrnaClaw scaffold for {title}."
    ...
    methodology_sections = "\n".join(
        (
            ...
            "3. Save stable ScrnaClaw outputs and summary artifacts.\n\n"
            "**Key parameters**\n\n"
...
- **Domain fit**: This scaffold targets the **{profile["title"]}** domain inside ScrnaClaw.
...
- Persist standard ScrnaClaw outputs such as `README.md`, `report.md`, `result.json`, and `reproducibility/`.
...
4. Write standard ScrnaClaw outputs and any domain-specific tables or plots.
...
- "Add a reusable ScrnaClaw skill called {skill_name}"
- "Create a {profile["title"].lower()} skill for {summary.lower()}"
- "Scaffold a new ScrnaClaw workflow around {methods_list[0]}"
...
\"\"\"Autogenerated ScrnaClaw scaffold for {title}.\"\"\"
...
This is an autogenerated ScrnaClaw skill scaffold.
...
This scaffold exists so the skill can be edited, reviewed, and iterated inside the ScrnaClaw repository.
...
def render_promoted_skill_script(
    *,
    ...
    return f"""#!/usr/bin/env python3
\"\"\"Promoted ScrnaClaw skill for {title}.\"\"\"
...
- Expand tests and tighten the ScrnaClaw output contract in follow-up edits.
\"\"\"
    ...
    *,
    ...
    if source_analysis_dir:
        ...
        if not resolved_source_dir.is_absolute():
            resolved_source_dir = (SCRNA_CLAW_DIR / resolved_source_dir).resolve()
    ...
    if not resolved_root.is_absolute():
        resolved_root = (SCRNA_CLAW_DIR / resolved_root).resolve()
