"""Agent-specific tools for the ScrnaClaw research pipeline.

Provides:
- ``think_tool`` — structured reflection (from EvoScientist)
- ``tavily_search`` — web search via Tavily (from EvoScientist)
- ``scrna_claw_execute`` — run ScrnaClaw analysis skills
...
def think_tool(reflection: str) -> str:
    ...
    When to use:
    - Before starting work: What do I know? What ScrnaClaw skills are available?
    ...
    3. ScrnaClaw skills — Is there a skill that fits the current task?
...
# =============================================================================
# ScrnaClaw skill execution tool
# =============================================================================


@tool(parse_docstring=True)
def scrna_claw_execute(
    skill: str,
    input_path: str = "",
    output_dir: str = "",
    method: str = "",
    ...
    """Execute an ScrnaClaw analysis skill.

    Runs a registered ScrnaClaw skill (e.g., spatial-preprocessing,
    sc-cell-annotation) with the specified parameters.

    Args:
        skill: Name of the ScrnaClaw skill to run
    ...
    # Locate scrna_claw.py script
    scrna_claw_dir = Path(__file__).resolve().parent.parent.parent
    script_path = scrna_claw_dir / "scrna_claw.py"

    if not script_path.exists():
        return f"Error: scrna_claw.py not found at {script_path}"

    # Load the script module
    spec = importlib.util.spec_from_file_location("_oc_script", script_path)
    if spec is None or spec.loader is None:
        return f"Error: cannot load scrna_claw.py"
...
_LIB_UTIL_NAMES: set[str] = {
    ...
    # Report helpers (from scrna_claw.common)
...
def skill_search(
    ...
    """Search ScrnaClaw's skill registry for available analysis functions.

    Use this BEFORE writing notebook code to find if an existing ScrnaClaw
    ...
    try:
        from scrna_claw.core.registry import OmicsRegistry
...
def notebook_create(
    ...
    The kernel has ScrnaClaw's project root on PYTHONPATH, so skill
    ...
    try:
        ...
        return (
            ...
            f"  Kernel running (python3), PYTHONPATH includes ScrnaClaw root.\n"
...
def notebook_add_execute(
    ...
    ScrnaClaw skill functions directly, e.g.::
...
def build_tool_registry() -> dict:
    ...
    return {
        ...
        "scrna_claw_execute": scrna_claw_execute,
