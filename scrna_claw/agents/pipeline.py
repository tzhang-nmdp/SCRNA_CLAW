...
from scrna_claw.agents.plan_state import (
...
from scrna_claw.agents.pipeline_result import (
    CompletionRunResult,
    PipelineRunResult,
    PlanRunResult,
)
from scrna_claw.core.provider_registry import get_langchain_llm
from scrna_claw.core.llm_timeout import build_llm_timeout_policy
from scrna_claw.runtime.task_store import (
...
_SCRNA_CLAW_ROOT = Path(__file__).resolve().parents[2]
...
PIPELINE_STAGE_DEFINITIONS: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        "id": "execute",
        "title": "Execute",
        "description": "Run ScrnaClaw skills in notebook",
        "owner": "coding-agent",
...
class ResearchPipeline:
    ...
    def __init__(
        ...
        self.hook_runtime = build_default_lifecycle_hook_runtime(_SCRNA_CLAW_ROOT)
    ...
    def _get_llm(self, agent_name: str | None = None):
        ...
        Global:
            3. ``LLM_PROVIDER`` / ``SCRNA_CLAW_MODEL``
    ...
    def _build_agent(self):
        """Build the deepagents agent with ScrnaClaw sub-agents.
        ...
        # Skills backend (read-only, maps /skills/ → ScrnaClaw skills/)
