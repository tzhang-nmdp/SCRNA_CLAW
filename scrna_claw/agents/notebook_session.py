"""Jupyter Notebook Session Manager for the ScrnaClaw Research Pipeline.

Provides ``NotebookSession`` — a persistent Jupyter kernel backed
``.ipynb`` editing and execution engine.  Adapted from CellVoyager's
``NotebookSession`` design, tailored for ScrnaClaw's multi-omics
...
# ---------------------------------------------------------------------------
# ScrnaClaw project root (for PYTHONPATH injection into kernel)
# ---------------------------------------------------------------------------
...
class NotebookSession:
    ...
    ``adata``) survive between steps — enabling ScrnaClaw skill functions
    ...
    def __init__(self, notebook_path: str, kernel_name: str = "python3"):
        ...
        bootstrap = (
            ...
            "\n"
            "# ── ScrnaClaw skill loader ──────────────────────────────\n"
            "def load_skill(name):\n"
            "    '''Load an ScrnaClaw skill module by name, e.g. load_skill(\"spatial-preprocess\").\n"
            ...
            "    return mod\n"
            "\n"
            "print('✓ ScrnaClaw kernel ready. Use load_skill(\"skill-name\") to load skills.')\n"
        )
