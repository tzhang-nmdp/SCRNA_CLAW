"""Custom backends for the ScrnaClaw research pipeline.

Adapts EvoScientist's backend architecture:
- ScrnaClawSandboxBackend: sandboxed shell execution + file ops
...
def create_sandbox_backend(workspace_dir: str):
    ...
    class ScrnaClawSandboxBackend(LocalShellBackend):
        ...
        def __init__(self, root_dir: str, **kwargs):
            ...
            self._sandbox_id = f"scrna_claw-{uuid.uuid4().hex[:8]}"
    ...
    return ScrnaClawSandboxBackend(workspace_dir)
...
def create_skills_backend(project_root: str):
    ...
    ----------
    project_root : str
        ScrnaClaw project root (parent of skills/).
    ...
    class ReadOnlySkillsBackend(FilesystemBackend):
        """Read-only access to ScrnaClaw skills definitions."""
