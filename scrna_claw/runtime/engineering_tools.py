...
def build_engineering_tool_specs() -> list[ToolSpec]:
    return [
        ToolSpec(
            name="tool_search",
            description=(
                "Search available ScrnaClaw tools by name, description, and tags. "
...
def build_engineering_tool_executors(
    *,
    ...
    root = Path(scrna_claw_dir).expanduser().resolve()
    ...
    async def web_fetch(args: dict[str, Any]) -> str:
        ...
        try:
            from scrna_claw.research.web_search import _fetch_webpage
...
def _resolve_state_root(state_root: str | Path | None) -> Path:
    if state_root is not None:
        ...
        root = (base / "scrna_claw" / "runtime").resolve()
...
def _preferred_read_root(
    *,
    ...
    if str(surface or "").strip().lower() == "interactive":
        return scrna_claw_dir
    fallback = scrna_claw_dir / "output" / "engineering"
...
def _read_roots(
    *,
    ...
    data_roots = [
        scrna_claw_dir / "data",
        scrna_claw_dir / "examples",
        scrna_claw_dir / "output",
    ]
    if str(surface or "").strip().lower() == "interactive":
        roots.append(scrna_claw_dir)
    roots.extend(data_roots)

    extra = os.environ.get("SCRNA_CLAW_DATA_DIRS", "").strip()
    if extra:
...
def _write_roots(
    *,
    ...
    if str(surface or "").strip().lower() == "interactive":
        return [scrna_claw_dir]
    fallback = (scrna_claw_dir / "output" / "engineering").resolve()
    fallback.mkdir(parents=True, exist_ok=True)
    return [fallback]
...
def _mcp_config_path() -> Path:
    xdg = os.environ.get("XDG_CONFIG_HOME")
    base = Path(xdg) if xdg else Path.home() / ".config"
    return (base / "scrna_claw" / "mcp.yaml").resolve()
