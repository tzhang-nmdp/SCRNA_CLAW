...
LOGGER = logging.getLogger("scrna_claw.runtime.context_layers")
...
1. Identity
   - You are a multi-omics analysis assistant powered by ScrnaClaw skills.
...
2. Existing-First Work
   ...
   - Prefer existing ScrnaClaw skills, workspace artifacts, and built-in utilities over ad hoc scripts or new abstractions.
...
1. Skill Routing and Capability
   {capability_rule}
   - If the user names an exact skill or gives an obvious exact skill invocation, call `scrna_claw` directly.
...
2. Method and Parameter Handling
   ...
   - If `sc-batch-integration` pauses because upstream preparation is recommended and the user explicitly agrees to continue with those prep steps, call `scrna_claw` again with `auto_prepare=true` instead of manually chaining separate tool calls yourself.
...
6. Controlled Execution and File Writing
   - Run analysis code via `scrna_claw` or `custom_analysis_execute`, not via generated shell scripts.
...
def load_knowhow_constraints(
    *,
    ...
    try:
        ...
        from scrna_claw.knowledge.knowhow import get_knowhow_injector
...
def _build_extension_prompt_pack_layer(request: ContextAssemblyRequest) -> ContextLayer | None:
    ...
    if not block and request.scrna_claw_dir:
        try:
            from scrna_claw.extensions import load_prompt_pack_runtime_context

            loader = request.extension_prompt_pack_loader or load_prompt_pack_runtime_context
            runtime_context = loader(
                request.scrna_claw_dir,
                surface=request.surface,
                skill=request.skill,
                query=request.query,
                domain=request.domain,
