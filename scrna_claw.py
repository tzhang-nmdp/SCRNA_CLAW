#!/usr/bin/env python3
"""ScrnaClaw — Multi-Omics Analysis Skills Runner.

Usage:
    python scrna_claw.py list
    python scrna_claw.py run <skill> --demo
    python scrna_claw.py run <skill> --input <data> --output <dir>
    python scrna_claw.py run spatial-pipeline --input <h5ad> --output <dir>
    python scrna_claw.py upload --input <data> --data-type <type>

Interactive CLI/TUI:
    python scrna_claw.py interactive               # Rich CLI (prompt_toolkit)
    python scrna_claw.py interactive --ui tui      # Full-screen Textual TUI
    python scrna_claw.py interactive -p "..."      # Single-shot mode
    python scrna_claw.py interactive --session <id> # Resume session
    python scrna_claw.py tui                       # Alias for --ui tui

MCP Server Management:
    python scrna_claw.py mcp list
    python scrna_claw.py mcp add <name> <command> [args]
    python scrna_claw.py mcp remove <name>
    python scrna_claw.py mcp config
"""
...
SCRNA_CLAW_DIR = Path(__file__).resolve().parent
SKILLS_DIR = SCRNA_CLAW_DIR / "skills"
EXAMPLES_DIR = SCRNA_CLAW_DIR / "examples"
DEFAULT_OUTPUT_ROOT = SCRNA_CLAW_DIR / "output"
SESSIONS_DIR = SCRNA_CLAW_DIR / "sessions"
...
def _get_write_analysis_notebook():
    from scrna_claw.common.notebook_export import write_analysis_notebook  # noqa: PLC0415
    return write_analysis_notebook
from scrna_claw.core.registry import registry
registry.load_all()
...
def _write_pipeline_readme(
    ...
    *,
    ...
    lines = [
        "# ScrnaClaw Pipeline Output Guide",
        "",
        "## Overview",
        "",
...
def upload_session(
    ...
    if str(SCRNA_CLAW_DIR) not in sys.path:
        sys.path.insert(0, str(SCRNA_CLAW_DIR))
    from scrna_claw.common.session import SpatialSession
...
def run_skill(
    ...
    *,
    ...
    try:
        import os
        env = os.environ.copy()
        env["PYTHONPATH"] = str(SCRNA_CLAW_DIR) + os.pathsep + env.get("PYTHONPATH", "")
        proc = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
...
class ScrnaClawParser(argparse.ArgumentParser):
    """Custom parser for beautiful ScrnaClaw CLI help output."""
    
    def print_help(self, file=None):
        if file is None:
            file = sys.stdout

        print(f"\n{BOLD}{CYAN}⬡ ScrnaClaw{RESET} — AI-powered Multi-Omics Analysis Platform\n", file=file)
        ...
        print(f"{DIM}ScrnaClaw project is under active development.{RESET}\n", file=file)
...
def main():
    ...
    parser = ScrnaClawParser(
        description="ScrnaClaw — Multi-Omics Skills Runner",
    ...
    # env
    env_p = sub.add_parser("env", help="Check installed ScrnaClaw dependency tiers")
    ...
    if args.command in ("interactive", "chat"):
        ...
        from scrna_claw.interactive.interactive import run_interactive
        run_interactive(
            workspace_dir=_ws,
    ...
    if args.command == "mcp":
        from scrna_claw.interactive._mcp import (
            list_mcp_servers,
            add_mcp_server,
            remove_mcp_server,
            MCP_CONFIG_PATH,
        ...
        if mcp_cmd == "list":
            ...
            if not servers:
                ...
                print(f"{CYAN}Add with: python scrna_claw.py mcp add <name> <command>{RESET}")
            else:
            ...
            from scrna_claw.interactive._mcp import remove_mcp_server
            ...
            print(f"Usage: python scrna_claw.py mcp [list|add|remove|config]")
            sys.exit(1)

    if args.command == "memory-server":
        import os
        if getattr(args, "host", None):
            os.environ["SCRNA_CLAW_MEMORY_HOST"] = args.host
        if getattr(args, "port", None):
            os.environ["SCRNA_CLAW_MEMORY_PORT"] = str(args.port)
        from scrna_claw.memory.server import main as _mem_main
        _mem_main()
        sys.exit(0)

    if args.command == "env":
        from scrna_claw.core.dependency_manager import get_installed_tiers
        tiers = get_installed_tiers()
        
        print(f"\n{BOLD}ScrnaClaw Environment Status{RESET}")
    ...
    if args.command == "doctor":
        from scrna_claw.diagnostics import build_doctor_report, render_doctor_report
    ...
    if args.command == "knowledge":
        ...
        if kb_cmd == "build":
            ...
            if "error" in stats:
                ...
                print(f"Run: python scrna_claw.py knowledge build")
            else:
            ...
            if not topics:
                print(f"{YELLOW}No topics found. Run: python scrna_claw.py knowledge build{RESET}")
            else:
            ...
            print("Usage: python scrna_claw.py knowledge [build|search|stats|list]")
            sys.exit(1)
