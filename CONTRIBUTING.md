...
ScrnaClaw uses **convention-over-configuration** for skill discovery. Place your files in the correct directory with the correct naming, and the system automatically handles registry, CLI, bot routing, and agent integration — no manual wiring needed.
...
python scrna_claw.py list | grep <skill-name>

# 2. Demo mode works
python scrna_claw.py run <skill-name> --demo --output /tmp/test_output
...
skills/<domain>/<name>/                 → registry.load_all() discovers the directory
    ...
    <name>.py (with main())             → registry.skills[alias] = {script, domain, description, ...}
                                          │
                                          ├→ CLI: `scrna_claw.py run <name>` works
...
3. Use `python scrna_claw.py list` to verify skills load correctly
...
Open an issue on [GitHub](https://github.com/TianGzlab/ScrnaClaw/issues) or check the documentation.
