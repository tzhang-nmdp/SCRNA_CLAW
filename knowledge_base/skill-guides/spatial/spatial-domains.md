...
title: ScrnaClaw Skill Guide — Spatial Domains
...
# ScrnaClaw Skill Guide — Spatial Domains
...
- which parameters matter first for ScrnaClaw's current wrapper
...
- For `spagcn`, `stagate`, and `graphst`, if `--n-domains` is omitted, ScrnaClaw defaults it to `7`.
...
Important warnings:
- ScrnaClaw will default `n_domains` to `7` if omitted, but this should still be explained to the user.
- Current ScrnaClaw SpaGCN flow does **not** use histology images; do not describe the run as image-aware.
