...
title: ScrnaClaw Skill Guide — Spatial Annotation
...
# ScrnaClaw Skill Guide — Spatial Annotation
...
- `tangram_train_genes` is an ScrnaClaw wrapper control for the gene list passed into `tg.pp_adatas(..., genes=...)`; it is not a separate Tangram API flag.
- `scanvi_max_epochs` currently drives SCVI pretraining, SCANVI finetuning, and query adaptation in the current wrapper.
- `model` is an ScrnaClaw wrapper path to a JSON marker-panel file for CellAssign, not a native CellAssign constructor argument.
...
- The current ScrnaClaw wrapper uses `mode="cells"` only. Do not promise cluster-mode Tangram behavior unless the implementation changes.
...
The score semantics are method-specific in the current ScrnaClaw outputs.
