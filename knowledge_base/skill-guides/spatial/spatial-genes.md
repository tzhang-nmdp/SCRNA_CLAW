...
title: ScrnaClaw Skill Guide — Spatially Variable Genes
...
# ScrnaClaw Skill Guide — Spatially Variable Genes
...
- `FlashS` currently exposes `--flashs-bandwidth` and `--flashs-n-rand-features` as **ScrnaClaw wrapper-level controls**; do not describe them as guaranteed upstream public API flags.
- If `--spatialde-no-aeh` is set, ScrnaClaw intentionally skips AEH pattern grouping even if AEH-specific arguments were supplied.
...
Important warnings:
- Do not treat `sparkx_max_genes` as a scientific parameter of SPARK-X itself; it is an ScrnaClaw wrapper-level scalability control.
- ScrnaClaw reports SPARK-X ranking as `-log10(p)` for readability, but significance should be interpreted using `qval` when available.
...
- Do not describe the FlashS score as a p-value. ScrnaClaw reports a wrapper score plus BH-adjusted `qval`.
