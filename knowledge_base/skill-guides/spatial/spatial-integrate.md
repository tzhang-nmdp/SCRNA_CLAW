...
title: ScrnaClaw Skill Guide — Spatial Integration
...
# ScrnaClaw Skill Guide — Spatial Integration
...
- which parameters matter first in ScrnaClaw's current wrapper
...
- **Current neighborhood / UMAP state**:
  - ScrnaClaw recomputes UMAP after integration, but the pre-integration graph still affects the baseline diagnostics
- **Cell ordering by batch**:
  - the Scanpy Scanorama wrapper expects same-batch cells to be contiguous; ScrnaClaw now sorts a temporary copy internally and restores original order afterward
...
- ScrnaClaw reports normalized batch-mixing entropy before and after integration.
- If `leiden` labels are missing after integration, ScrnaClaw computes them once for visualization and reporting.
...
- ScrnaClaw automatically clamps `bbknn_n_pcs` to the number of PCs actually present in `X_pca`.
...
Important warnings:
- The current ScrnaClaw flow uses the Scanpy wrapper on `X_pca`, not direct raw-expression integration.
...
- **Python standard gallery**: this is the canonical ScrnaClaw result layer. It should be the first thing users inspect.
...
description. The representation and failure modes differ in current ScrnaClaw.
