...
title: ScrnaClaw Skill Guide — Spatial Differential Expression
...
# ScrnaClaw Skill Guide — Spatial Differential Expression
...
- **Group overlap across samples**:
  - if the same samples contribute to both groups, ScrnaClaw can use a paired
    design in the PyDESeq2 path
  - if samples belong to only one group each, ScrnaClaw uses `~ condition`
...
- ScrnaClaw does **not** random-split cells into fake DESeq2 replicates.
- If `groupby=leiden` is missing, the wrapper can auto-compute Leiden labels.
- ScrnaClaw emits a canonical Python gallery under `figures/` plus
...
Current ScrnaClaw `spatial-de` separates visualization into two layers:
...
explanation. In current ScrnaClaw they answer different scientific questions.
