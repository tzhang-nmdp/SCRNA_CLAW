...
title: ScrnaClaw Skill Guide — Spatial Statistics
...
# ScrnaClaw Skill Guide — Spatial Statistics
...
- **Expression representation**: current ScrnaClaw uses `adata.X` for Moran,
...
- Graph-based methods reuse an existing `spatial_connectivities` graph by
  default.
- If the user changes `stats_n_neighs` or `stats_n_rings`, ScrnaClaw rebuilds
  the graph to honor that request.
- If the default `cluster_key=leiden` is missing and the method requires it,
  ScrnaClaw can run a minimal clustering pass to create it.
...
2. If the user changes `stats_n_neighs` or `stats_n_rings`, tell them ScrnaClaw
