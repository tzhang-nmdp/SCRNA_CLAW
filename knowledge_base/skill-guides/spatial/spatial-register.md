...
title: ScrnaClaw Skill Guide — Spatial Register
...
# ScrnaClaw Skill Guide — Spatial Register
...
About to run spatial registration
  ...
  Note: This is a 4-slice dataset, so PASTE is the more appropriate current ScrnaClaw wrapper choice than STalign.
```
...
Important warning:
- In the current wrapper, `use_expression` is not a raw upstream STalign switch; it controls whether ScrnaClaw constructs a PC1-derived intensity image before calling LDDMM.
...
- Refer to `spatial_aligned` as the aligned coordinate space exported by ScrnaClaw.
