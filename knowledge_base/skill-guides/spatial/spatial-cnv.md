...
title: ScrnaClaw Skill Guide — Spatial CNV
...
# ScrnaClaw Skill Guide — Spatial CNV
...
- **Reference metadata**:
  ...
  - current ScrnaClaw `numbat` wrapper requires `reference_key` and
    `reference_cat`
...
- `infercnvpy` runs on `adata.X`, then ScrnaClaw computes CNV PCA, neighbors,
  Leiden clusters, and `cnv_score`.
- `numbat` exports a lightweight h5ad plus an allele-count CSV into the R
  subprocess.
- Current ScrnaClaw `numbat` support is best viewed as a structured wrapper
  around the official `run_numbat()` path; it is more dependency-sensitive than
  the default `infercnvpy` workflow.
- ScrnaClaw defaults `infercnv_n_jobs=1` for reproducibility, even though
...
- ScrnaClaw excludes `chrX` / `chrY` by default because infercnvpy documents
...
- In the current ScrnaClaw wrapper, `reference_key` and `reference_cat` are
...
Current ScrnaClaw `spatial-cnv` separates visualization into two layers:
