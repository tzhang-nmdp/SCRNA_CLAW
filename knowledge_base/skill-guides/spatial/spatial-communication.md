...
title: ScrnaClaw Skill Guide — Spatial Communication
...
# ScrnaClaw Skill Guide — Spatial Communication
...
- **Species**:
  ...
  - `cellphonedb` and `fastccc` are human-only in current ScrnaClaw
...
- ScrnaClaw standardizes all methods into a canonical LR table with `ligand`, `receptor`, `source`, `target`, `score`, and `pvalue`.
...
Important warnings:
- Current ScrnaClaw CellPhoneDB wrapper is human-only.
...
Important warnings:
- In current ScrnaClaw, FastCCC is human-only.
...
- If the user insists on CellPhoneDB/FastCCC for mouse, explain that current ScrnaClaw implementation does not support that combination yet.
...
Current ScrnaClaw `spatial-communication` separates visualization into two
layers:
...
story. In current ScrnaClaw, they expose different statistical behaviors and
