...
title: ScrnaClaw Skill Guide — Spatial Deconvolution
...
# ScrnaClaw Skill Guide — Spatial Deconvolution
...
- which parameters matter first in the current ScrnaClaw wrapper
...
- `flashdeconv` parameters exposed by ScrnaClaw are taken from the public Python API; current matrix-assumption guidance remains intentionally conservative because the package is newer.
- ScrnaClaw emits a canonical Python gallery under `figures/` plus `figure_data/`
...
Important warning:
- `cell2location_n_epochs` is wrapper-level training budget language in ScrnaClaw; the public biological priors that users usually care about first are `N_cells_per_location` and `detection_alpha`.
...
- Start with `spotlight_n_top=50` in ScrnaClaw for a modest first pass.
...
Important warning:
- `spotlight_n_top` is the public SPOTlight argument, but the current default of 50 is an ScrnaClaw first-pass wrapper choice rather than the package's only defensible setting.
...
Current ScrnaClaw `spatial-deconv` separates visualization into two layers:
...
constraints in current ScrnaClaw.
