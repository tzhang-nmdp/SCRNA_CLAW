...
title: ScrnaClaw Skill Guide — Spatial Enrichment
...
# ScrnaClaw Skill Guide — Spatial Enrichment

**Status**: implementation-aligned guide derived from the current ScrnaClaw
...
- **Gene-set source**:
  - local, deterministic first pass: `source=scrna_claw_core`
...
Important implementation notes in current ScrnaClaw:
...
- If the requested external library cannot be resolved, ScrnaClaw falls back to
...
|--------|----------------|----------------------------|-------------|
| **Enrichr** | Fast biological interpretation of positive marker genes | `source=scrna_claw_core`, `de_method=wilcoxon`, `enrichr_padj_cutoff=0.05`, `enrichr_log2fc_cutoff=1.0`, `enrichr_max_genes=200` | Depends strongly on the marker thresholding step |
...
| **ssGSEA** | Compare group-level pathway score profiles rather than term-level p-values | `source=scrna_claw_core`, `ssgsea_sample_norm_method=rank`, `ssgsea_correl_norm_type=rank`, `ssgsea_min_size=15`, `ssgsea_max_size=500`, `ssgsea_weight=0.25` | Current wrapper is group-level mean-profile scoring, not per-spot scoring |
...
- Start with `source=scrna_claw_core` for a deterministic local-first pass.
...
- Start with `gsea_ranking_metric=auto`; ScrnaClaw prefers `scores`, then
  `logfoldchanges`.
...
- Explain clearly that current ScrnaClaw ssGSEA is **group-level**.
...
1. If the user wants a stable, local-first run, start with `scrna_claw_core`.
...
Current ScrnaClaw `spatial-enrichment` separates visualization into two layers:
...
table" explanation. In current ScrnaClaw they answer different questions.
