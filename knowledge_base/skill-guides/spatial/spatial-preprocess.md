...
title: ScrnaClaw Skill Guide — Spatial Preprocess
...
# ScrnaClaw Skill Guide — Spatial Preprocess
...
Current ScrnaClaw `spatial-preprocess` runs:
...
Important warning:
- Tissue presets are ScrnaClaw wrapper knowledge, not upstream Scanpy APIs.
...
Important warning:
- In current ScrnaClaw, HVG selection uses `flavor="seurat_v3"` on the counts layer, not on log-normalized `adata.X`.
...
also stating the resolved thresholds. In current ScrnaClaw, the effective
