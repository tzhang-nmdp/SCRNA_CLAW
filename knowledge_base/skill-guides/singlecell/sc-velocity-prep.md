...
title: ScrnaClaw Skill Guide — SC Velocity Prep
...
# ScrnaClaw Skill Guide — SC Velocity Prep

**Status**: implementation-aligned guide for the current
`sc-velocity-prep` wrapper. It explains how ScrnaClaw creates velocity-ready
...
Current ScrnaClaw `sc-velocity-prep` does:
...
Current ScrnaClaw `sc-velocity-prep` does **not**:
...
That is exactly why ScrnaClaw’s current STARsolo velocity path stays close to
...
- ScrnaClaw aligns by shared cells and shared genes
- the merge is an intersection, not a union
- if barcodes differ only by suffix conventions such as `-1`, ScrnaClaw tries a lightweight normalization pass
