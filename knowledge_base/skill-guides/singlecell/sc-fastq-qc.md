...
title: ScrnaClaw Skill Guide — SC FASTQ QC
...
# ScrnaClaw Skill Guide — SC FASTQ QC

**Status**: implementation-aligned guide for the current `sc-fastq-qc`
wrapper. It explains how ScrnaClaw combines mainstream FastQC / MultiQC usage
...
Current ScrnaClaw `sc-fastq-qc` does:
...
Current ScrnaClaw `sc-fastq-qc` does **not**:
...
- If they were unavailable: explain that the ScrnaClaw fallback summary is still valid for a lightweight local screen, but it is not a full FastQC replacement.
