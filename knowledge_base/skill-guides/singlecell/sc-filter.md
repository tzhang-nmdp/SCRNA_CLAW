...
title: ScrnaClaw Skill Guide — SC Filter
...
# ScrnaClaw Skill Guide — SC Filter
...
wrapper-specific caveats, not a claim that ScrnaClaw already performs automatic
QC decision-making.
...
- `tissue` is an ScrnaClaw wrapper preset, not a Scanpy upstream parameter
- `min_genes`, `max_mt_percent`, and `min_cells` are the primary first-pass controls

## Step 2: Pick The Method Deliberately

Current ScrnaClaw exposes one filtering workflow:
