...
title: ScrnaClaw Skill Guide — SC Standardize Input
...
# ScrnaClaw Skill Guide — SC Standardize Input
...
- the user uploads an external `.h5ad` from outside ScrnaClaw
...
If the input is already an ScrnaClaw-standardized `h5ad`, you usually do not need to rerun this step. Many downstream scRNA skills should canonicalize compatible inputs automatically.
...
6. record provenance and state in:
   - `adata.uns['scrna_claw_input_contract']`
   - `adata.uns['scrna_claw_matrix_contract']`
