...
- **Do not overstate Cell Ranger coverage**: upstream `cellranger count` can also handle Feature Barcode modes, but the current ScrnaClaw wrapper is intentionally scoped to the core count workflow and stable matrix import path.
...
- **Explain the hand-off artifact**: `processed.h5ad` is the downstream ScrnaClaw contract, but it does not mean all upstream backend details have been discarded.
