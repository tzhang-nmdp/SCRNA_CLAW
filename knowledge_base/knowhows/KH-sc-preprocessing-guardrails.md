...
- **Do not fake method choice**: current ScrnaClaw `sc-preprocessing` exposes four real backends, `scanpy`, `seurat`, `sctransform`, and `pearson_residuals`; do not imply more methods exist, and do not silently replace an explicitly requested method with another backend.
