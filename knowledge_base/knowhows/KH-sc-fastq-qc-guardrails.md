...
- **Be explicit about the current wrapper fallback**: ScrnaClaw tries FastQC and MultiQC when those binaries are installed, but it always writes its own sampled Python summary so the report remains stable in minimal environments.
