...
- **Check species support before promising a method**: current ScrnaClaw wrapper supports human and mouse globally, but `cellphonedb` and `fastccc` are effectively human-only.
...
- **Preserve the standardized output contract**: method-specific backends may differ internally, but ScrnaClaw exports a canonical LR table plus communication summary and signaling-role tables.
