...
- **Do not invent hidden knobs**: official CellBender docs also discuss `total-droplets-included`, but the current ScrnaClaw wrapper does not expose it.
- **Describe outputs honestly**: for CellBender, treat `cellbender_output/` as the native method output bundle and `corrected.h5ad` as ScrnaClaw's downstream-ready wrapper export.
