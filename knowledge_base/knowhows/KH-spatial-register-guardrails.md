...
- **Respect slice-count constraints**: current ScrnaClaw STalign wrapper is pairwise-only; PASTE is the correct current wrapper choice for 3 or more slices.
...
- **Separate upstream from wrapper controls**: `paste_alpha`, `paste_dissimilarity`, and `paste_use_gpu` are public PASTE controls, while `stalign_image_size` and `use_expression` are current ScrnaClaw wrapper-level controls around the STalign workflow.
- **Preserve the output contract**: aligned coordinates must land in `adata.obsm["spatial_aligned"]`, with figures and metrics exported through the standard ScrnaClaw layout.
