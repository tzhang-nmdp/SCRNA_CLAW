...
ScrnaClaw is a local-first, skill-based multi-omics platform with three main execution surfaces:

- CLI commands via `oc` / `python scrna_claw.py`
...
- A shared prompt/context runtime in `scrna_claw/runtime/`
...
Input data / user intent
    ...
    -> skill execution (`scrna_claw` tool or direct CLI)
...
```text
ScrnaClaw/
|-- scrna_claw.py                 # Repository-root CLI launcher
|-- scrna_claw/
...
Each ScrnaClaw skill is a reusable analysis unit with:

- `SKILL.md` metadata and method contract
- A Python implementation with a stable CLI shape
- Optional tests and demo data

Skills are discovered from `skills/` and indexed through `scrna_claw.core.registry`. Shared domain utilities live under per-domain `_lib/` packages and are not treated as user-callable skills.
...
The repository-root `scrna_claw.py` remains the canonical launcher. The package script `oc` resolves and delegates to it through `scrna_claw/cli.py`.
...
Interactive sessions are implemented in `scrna_claw/interactive/` and share the same backend loop:
...
- The LLM tool loop
- Runtime integration with `scrna_claw/runtime/`
- audit logging
...
The shared runtime lives in `scrna_claw/runtime/` and is organized into four major layers:
...
`assemble_chat_context()` in `scrna_claw/runtime/context_assembler.py` prepares the prompt-facing state for each turn. It now performs concurrent preparation of multiple context sources, including:
...
The assembled prompt is built through ordered context layers from `scrna_claw/runtime/context_layers/`.
...
The prompt layers encode operational rules for ScrnaClaw's current design:
...
ScrnaClaw now includes explicit context-budget controls in the runtime rather than relying on raw transcript growth.

### Multi-stage compaction

`scrna_claw/runtime/context_compaction.py` implements five compaction stages:
...
`scrna_claw/runtime/query_engine.py` tracks `has_attempted_reactive_compact` so a turn only performs one emergency retry after a prompt-too-long response from the model API.

### Token budget continuation

`scrna_claw/runtime/token_budget.py` implements a per-turn token budget tracker. When a budget is configured, the runtime can inject a continuation nudge telling the model to keep working on the same task instead of stopping early. It also stops nudging once progress meaningfully slows down.
...
This is the main mechanism ScrnaClaw uses for "tool result budget" management.
...
For each tool request, ScrnaClaw performs:
...
This gives ScrnaClaw a Claude-Code-style structured execution path while staying aligned with the project's existing Python runtime.

## Memory Model

ScrnaClaw uses two distinct but complementary memory layers.

### 1. Graph memory

Implemented under `scrna_claw/memory/`, graph memory stores durable cross-session context such as:
...
Scoped memory is workspace-local memory stored under `.scrna_claw/scoped_memory` and indexed separately from the graph store. It is intended for:
...
Interactive sessions can load MCP server definitions from the ScrnaClaw config. Prompt injection is budget-aware:
...
The extension system under `scrna_claw/extensions/` supports activation surfaces such as:
...
Typical ScrnaClaw outputs include:
...
ScrnaClaw is no longer just a collection of skills behind a CLI. The current architecture is a layered runtime with:
