...
ScrnaClaw uses two complementary memory layers:
...
## Why ScrnaClaw Has Two Memory Layers
...
Graph memory lives under `scrna_claw/memory/` and is backed by SQLite or PostgreSQL through SQLAlchemy.
...
This allows ScrnaClaw to preserve relationships such as:
...
During chat-context assembly, ScrnaClaw can load session memory for the active `user_id/platform/chat_id` tuple and inject it as prompt memory context.

### Scoped-memory recall

When a workspace or pipeline workspace is active, ScrnaClaw can recall matching scoped-memory records and inject a summarized `## Scoped Memory` block into the prompt.
...
| Without memory | With ScrnaClaw memory |
| --- | --- |
...
ScrnaClaw ships with a memory dashboard frontend plus a REST API backend.

### 1. Start the backend API

The API server runs through the ScrnaClaw CLI:
...
If you want to bind it to a non-local interface, you must also set `SCRNA_CLAW_MEMORY_API_TOKEN`.
...
```python
import asyncio
from scrna_claw.memory import MemoryClient, SessionContext
...
```python
from scrna_claw.memory import write_scoped_memory
...
Interactive CLI and TUI sessions expose scoped-memory management through slash-command support in `scrna_claw/interactive/_memory_command_support.py`.
...
ScrnaClaw's memory model is intentionally layered:
