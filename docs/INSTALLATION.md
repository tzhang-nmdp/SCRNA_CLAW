...
ScrnaClaw uses optional dependency groups instead of requiring one monolithic environment.
...
ScrnaClaw automatically reads the project-root `.env` file for interactive startup. If `python-dotenv` is not installed, a built-in fallback parser is used, so standard `.env` key/value files still work.
...
By default this binds to `127.0.0.1:8766`. If you bind the memory API to a non-local interface, you must also set `SCRNA_CLAW_MEMORY_API_TOKEN`.
...
- ScrnaClaw stores MCP configuration in the user config directory
...
### `oc` cannot find `scrna_claw.py`

The package entrypoint searches for the repository-root launcher. Run `oc` from inside the ScrnaClaw repository, or set:

```bash
export SCRNA_CLAW_CLI_PATH=/absolute/path/to/scrna_claw.py
```
...
This matches ScrnaClaw's current modular runtime design much better than trying to install every optional method on day one.
