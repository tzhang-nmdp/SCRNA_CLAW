...
```bash
python scrna_claw.py run preprocess \
...
```bash
# leiden (default)
python scrna_claw.py run domains \
...
python scrna_claw.py run annotate \
...
python scrna_claw.py run deconv \
...
```bash
# From the ScrnaClaw project root:
...
python scrna_claw.py run statistics \
...
```bash
python scrna_claw.py run genes \
...
```bash
python scrna_claw.py run condition \
...
```bash
python scrna_claw.py run communication \
...
```bash
python scrna_claw.py run velocity \
...
```bash
python scrna_claw.py run trajectory \
...
```bash
python scrna_claw.py run enrichment \
...
```bash
python scrna_claw.py run integrate \
...
```bash
python scrna_claw.py run register \
...
python scrna_claw.py run orchestrator \
...
```bash
python scrna_claw.py run genomics-qc --demo
python scrna_claw.py run genomics-qc --input reads.fastq.gz --output output/qc
```
...
```bash
python scrna_claw.py run genomics-alignment --demo
python scrna_claw.py run genomics-alignment --input aligned.sam --output output/alignment
```
...
```bash
python scrna_claw.py run genomics-variant-calling --demo
python scrna_claw.py run genomics-variant-calling --input sample.bam --output output/variants
```
...
```bash
python scrna_claw.py run genomics-vcf-operations --demo
python scrna_claw.py run genomics-vcf-operations --input vars.vcf --output output/vcf-ops \
...
```bash
python scrna_claw.py run genomics-variant-annotation --demo
python scrna_claw.py run genomics-variant-annotation --input annotated.csv --output output/annotation
```
...
```bash
python scrna_claw.py run genomics-sv-detection --demo
python scrna_claw.py run genomics-sv-detection --input structural.vcf --output output/sv
```
...
```bash
python scrna_claw.py run genomics-cnv-calling --demo
python scrna_claw.py run genomics-cnv-calling --input bins.csv --output output/cnv --alpha 0.01
```
...
```bash
python scrna_claw.py run genomics-assembly --demo
python scrna_claw.py run genomics-assembly --input contigs.fasta --output output/assembly \
  --genome-size 5000000
```
...
```bash
python scrna_claw.py run genomics-phasing --demo
python scrna_claw.py run genomics-phasing --input phased.vcf --output output/phasing
```
...
```bash
python scrna_claw.py run genomics-epigenomics --demo
python scrna_claw.py run genomics-epigenomics --input peaks.narrowPeak --output output/epi \
  --assay atac-seq
```
...
```bash
# Demo
python scrna_claw.py run metabolomics-peak-detection --demo

# With data
python scrna_claw.py run metabolomics-peak-detection \
...
```bash
python scrna_claw.py run metabolomics-xcms-preprocessing --demo

python scrna_claw.py run metabolomics-xcms-preprocessing \
...
```bash
python scrna_claw.py run metabolomics-normalization --demo --method quantile

python scrna_claw.py run metabolomics-normalization \
...
```bash
python scrna_claw.py run metabolomics-annotation --demo

python scrna_claw.py run metabolomics-annotation \
...
```bash
python scrna_claw.py run metabolomics-quantification --demo --impute knn

python scrna_claw.py run metabolomics-quantification \
...
```bash
python scrna_claw.py run metabolomics-statistics --demo --method ttest

python scrna_claw.py run metabolomics-statistics \
...
```bash
python scrna_claw.py run metabolomics-de --demo

python scrna_claw.py run metabolomics-de \
...
```bash
python scrna_claw.py run metabolomics-pathway-enrichment --demo

python scrna_claw.py run metabolomics-pathway-enrichment \
...
```bash
python scrna_claw.py run bulkrna-read-qc --demo
python scrna_claw.py run bulkrna-read-qc --input reads.fastq.gz --output output/bulk-fastqc
```
...
```bash
python scrna_claw.py run bulkrna-read-alignment --demo
python scrna_claw.py run bulkrna-read-alignment --input Log.final.out --output output/bulk-align
```
...
```bash
python scrna_claw.py run bulkrna-qc --demo
python scrna_claw.py run bulkrna-qc --input counts.csv --output output/bulk-qc
```
...
```bash
python scrna_claw.py run bulkrna-geneid-mapping --demo
python scrna_claw.py run bulkrna-geneid-mapping --input counts.csv \
...
```bash
python scrna_claw.py run bulkrna-batch-correction --demo
python scrna_claw.py run bulkrna-batch-correction --input counts.csv \
...
python scrna_claw.py run bulkrna-de --input counts.csv --output output/bulk-de \
...
```bash
python scrna_claw.py run bulkrna-splicing --demo
python scrna_claw.py run bulkrna-splicing --input splicing_events.csv --output output/bulk-splicing
```
...
```bash
python scrna_claw.py run bulkrna-enrichment --demo
python scrna_claw.py run bulkrna-enrichment --input de_results.csv --output output/bulk-enrich
```
...
```bash
python scrna_claw.py run bulkrna-deconvolution --demo
python scrna_claw.py run bulkrna-deconvolution --input counts.csv --output output/bulk-deconv
```
...
python scrna_claw.py run bulkrna-coexpression --input counts.csv --output output/bulk-wgcna
```
...
```bash
python scrna_claw.py run bulkrna-ppi-network --demo
python scrna_claw.py run bulkrna-ppi-network --input de_results.csv --output output/bulk-ppi \
...
```bash
python scrna_claw.py run bulkrna-survival --demo
python scrna_claw.py run bulkrna-survival --input counts.csv \
...
```bash
python scrna_claw.py run bulkrna-trajblend --demo
python scrna_claw.py run bulkrna-trajblend --input bulk_counts.csv \
