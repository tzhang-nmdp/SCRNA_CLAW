#!/usr/bin/env python3
"""
run.py — ScrnaClaw Multi-Channel Runner
========================================
...
from scrna_claw.common.runtime_env import load_env_file
from scrna_claw.core.provider_registry import detect_provider_from_env, resolve_provider
...
logger = logging.getLogger("scrna_claw.runner")
...
def _resolve_bootstrap_llm_config(
    ...
    resolved_url, resolved_model, resolved_key = resolve_provider(
        ...
        model=str(source.get("SCRNA_CLAW_MODEL", source.get("SPATIALCLAW_MODEL", "")) or ""),
...
async def _run_channels(channel_names: list[str], health_port: int = 0) -> None:
    ...
    print(
        f"ScrnaClaw bot running with {len(running)} channel(s): "
    ...
    try:
        ...
        print("ScrnaClaw bot stopped.")


def main():
    parser = argparse.ArgumentParser(
        description="ScrnaClaw Multi-Channel Bot Runner",
