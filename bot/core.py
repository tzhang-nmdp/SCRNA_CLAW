"""
core.py — ScrnaClaw Bot shared engine
=====================================
Platform-independent logic shared by ScrnaClaw messaging frontends:
...
from scrna_claw.common.runtime_env import load_project_dotenv
from scrna_claw.core.llm_timeout import (
...
# ---------------------------------------------------------------------------
# Paths (relative to ScrnaClaw project root)
# ---------------------------------------------------------------------------

SCRNA_CLAW_DIR = Path(__file__).resolve().parent.parent
load_project_dotenv(SCRNA_CLAW_DIR, override=False)
SCRNA_CLAW_PY = SCRNA_CLAW_DIR / "scrna_claw.py"
OUTPUT_DIR = SCRNA_CLAW_DIR / "output"
DATA_DIR = SCRNA_CLAW_DIR / "data"
EXAMPLES_DIR = SCRNA_CLAW_DIR / "examples"
PYTHON = sys.executable

MAX_UPLOAD_BYTES = 50 * 1024 * 1024
MAX_PHOTO_BYTES = 20 * 1024 * 1024

if str(SCRNA_CLAW_DIR) not in sys.path:
    sys.path.insert(0, str(SCRNA_CLAW_DIR))
from scrna_claw.common.report import build_output_dir_name
from scrna_claw.common.user_guidance import (
...
from scrna_claw.core.registry import ensure_registry_loaded, registry
from scrna_claw.runtime.bot_tools import BotToolContext, build_bot_tool_registry
from scrna_claw.runtime.context_assembler import assemble_chat_context as _assemble_chat_context
from scrna_claw.runtime.engineering_tools import build_engineering_tool_executors
from scrna_claw.runtime.query_engine import (
    QueryEngineCallbacks,
    QueryEngineConfig,
    QueryEngineContext,
    run_query_engine,
)
from scrna_claw.runtime.hooks import build_default_lifecycle_hook_runtime
from scrna_claw.runtime.policy import TOOL_POLICY_ALLOW
from scrna_claw.runtime.policy_state import ToolPolicyState
from scrna_claw.runtime.system_prompt import build_system_prompt, get_role_guardrails
from scrna_claw.runtime.tool_orchestration import (
    EXECUTION_STATUS_POLICY_BLOCKED,
    ToolExecutionRequest,
)
from scrna_claw.runtime.tool_result_store import ToolResultStore
...
from scrna_claw.runtime.tool_spec import PROGRESS_POLICY_ANALYSIS
from scrna_claw.runtime.verification import format_completion_mapping_summary
...
logger = logging.getLogger("scrna_claw.bot")
...
def format_skills_table(plain: bool = False) -> str:
    ...
    if plain:
        lines = [f"ScrnaClaw Skills ({total} total)", "=" * 40, ""]
    else:
        lines = [f"🔬 ScrnaClaw Skills ({total} total)", ""]
...
_AUDIT_LOG_DIR = SCRNA_CLAW_DIR / "bot" / "logs"
...
SCRNA_CLAW_MODEL: str = "deepseek-chat"
LLM_PROVIDER_NAME: str = ""

MAX_HISTORY = int(os.getenv("SCRNA_CLAW_MAX_HISTORY", "50"))
MAX_HISTORY_CHARS = int(os.getenv("SCRNA_CLAW_MAX_HISTORY_CHARS", "0"))
MAX_CONVERSATIONS = int(os.getenv("SCRNA_CLAW_MAX_CONVERSATIONS", "1000"))
TOOL_RESULT_INLINE_BYTES = int(os.getenv("SCRNA_CLAW_TOOL_RESULT_INLINE_BYTES", "6000"))
TOOL_RESULT_PREVIEW_CHARS = int(os.getenv("SCRNA_CLAW_TOOL_RESULT_PREVIEW_CHARS", "1200"))
TOOL_RESULT_STORAGE_DIR = SCRNA_CLAW_DIR / "bot" / "logs" / "tool_results"
...
def get_usage_snapshot() -> dict:
    ...
    inp_price, out_price = _get_token_price(SCRNA_CLAW_MODEL)
    cost = (
...
async def _auto_capture_dataset(session_id: str, input_path: str, data_type: str = ""):
    ...
    try:
        from scrna_claw.memory.compat import DatasetMemory

        # Make path relative to project dir if possible
        try:
            rel_path = str(Path(input_path).relative_to(SCRNA_CLAW_DIR))
        except ValueError:
...
async def _auto_capture_analysis(session_id: str, skill: str, args: dict, output_dir: Path, success: bool):
    ...
    try:
        from scrna_claw.memory.compat import AnalysisMemory
...
def init(
    ...
    """
    global llm, SCRNA_CLAW_MODEL, LLM_PROVIDER_NAME, memory_store, session_manager
    ...
    SCRNA_CLAW_MODEL = resolved_model
    ...
    logger.info(
        ...
        f"model={SCRNA_CLAW_MODEL}, base_url={resolved_url or '(default)'}"
    )

    # Memory initialization — uses the new graph-based memory system
    # Enabled by default; disable with SCRNA_CLAW_MEMORY_ENABLED=false
    if os.getenv("SCRNA_CLAW_MEMORY_ENABLED", "true").lower() not in ("false", "0", "no"):
        try:
            from scrna_claw.memory.compat import CompatMemoryStore

            db_url = os.getenv("SCRNA_CLAW_MEMORY_DB_URL")  # None = use default (~/.config/scrna_claw/memory.db)
            ...
            logger.info("Graph memory system initialized (scrna_claw.memory)")
        except ImportError:
...
def _ensure_system_prompt():
    ...
    if not SYSTEM_PROMPT:
        SYSTEM_PROMPT = build_system_prompt(scrna_claw_dir=str(SCRNA_CLAW_DIR))
...
def resolve_dest(folder: str | None, default: Path | None = None) -> Path:
    ...
    if not dest.is_absolute():
        dest = SCRNA_CLAW_DIR / dest
    try:
        dest.resolve().relative_to(SCRNA_CLAW_DIR.resolve())
    except ValueError:
...
def _build_trusted_dirs() -> list[Path]:
    ...
    extra = os.environ.get("SCRNA_CLAW_DATA_DIRS", os.environ.get("SPATIALCLAW_DATA_DIRS", ""))
    if extra:
        for d in extra.split(","):
            ...
            if d:
                ...
                if p.is_absolute() and p.is_dir():
                    ...
                    logger.warning(f"SCRNA_CLAW_DATA_DIRS: ignoring '{d}' (not an absolute directory)")
    return dirs
...
def validate_input_path(filepath: str, *, allow_dir: bool = False) -> Path | None:
    ...
    try:
        resolved.relative_to(SCRNA_CLAW_DIR.resolve())
        return resolved
    except ValueError:
        pass
...
def _inspect_h5ad_integration_readiness(file_path: Path) -> dict:
    ...
    adata = ad.read_h5ad(file_path, backed="r")
    try:
        contract = adata.uns.get("scrna_claw_input_contract", {})
...
async def _run_omics_skill_step(
    *,
    ...
    cmd = [PYTHON, str(SCRNA_CLAW_PY), "run", skill_key]
    if mode == "demo":
        cmd.append("--demo")
    elif input_path:
...
async def _auto_prepare_sc_batch_integration(
    *,
    ...
    final_result = await execute_scrna_claw(chained_args, session_id=session_id, chat_id=chat_id)
    return prefix + "\n\n---\n" + final_result
...
def _build_method_preview(
    *,
    ...
    # metadata.scrna_claw.param_hints.<method>.requires
...
async def execute_scrna_claw(args: dict, session_id: str = None, chat_id: int | str = 0) -> str:
    """Execute an ScrnaClaw skill via subprocess (waits until completion)."""
        ...
        try:
            ...
            if decision.chosen_skill:
                if getattr(decision, "should_create_skill", False):
                    return (
                        "This request is asking to add a reusable ScrnaClaw skill.\n\n"
                ...
                return (
                    "No existing ScrnaClaw skill fully matches this request.\n"
    ...
    # Build command
    cmd = [PYTHON, str(SCRNA_CLAW_PY), "run"]
    ...
    try:
        from scrna_claw.knowledge.resolver import AdvisoryEvent, get_resolver
...
async def execute_parse_literature(args: dict) -> str:
    ...
    # Build command
    lit_script = SCRNA_CLAW_DIR / "skills" / "literature" / "literature_parse.py"
...
async def execute_fetch_geo_metadata(args: dict) -> str:
    ...
    sys.path.insert(0, str(SCRNA_CLAW_DIR / "skills" / "literature"))
    try:
...
async def execute_inspect_data(args: dict) -> str:
    ...
    if preview_params and method_arg:
        ...
        if preview_block:
            ...
            lines.append("- Add method hints in SKILL.md: `metadata.scrna_claw.param_hints.<method>`.")
...
async def execute_remember(args: dict, session_id: str = None) -> str:
    ...
    if not memory_store:
        return "Memory system not enabled. Set SCRNA_CLAW_MEMORY_BACKEND=sqlite in .env"
    if not session_id:
    ...
    try:
        if mem_type == "preference":
            from scrna_claw.memory.compat import PreferenceMemory
            ...
            from scrna_claw.memory.compat import InsightMemory
            ...
            from scrna_claw.memory.compat import ProjectContextMemory
...
async def execute_consult_knowledge(args: dict, **kwargs) -> str:
    """Query the ScrnaClaw knowledge base for analysis guidance."""
    try:
        import time as _t
        _ck_start = _t.monotonic()

        from scrna_claw.knowledge import KnowledgeAdvisor
        from scrna_claw.knowledge.semantic_bridge import (
        ...
        if not advisor.ensure_available(auto_build=True):
            return "Knowledge base not built yet. Run: python scrna_claw.py knowledge build"
        ...
        try:
            from scrna_claw.knowledge.telemetry import get_telemetry
...
async def execute_create_omics_skill(args: dict, **kwargs) -> str:
    """Create a new ScrnaClaw skill scaffold inside the repository."""
    try:
        from scrna_claw.core.skill_scaffolder import create_skill_scaffold
        ...
        return (
            "Created ScrnaClaw skill scaffold.\n"
        ...
        return f"Error creating ScrnaClaw skill: {e}"
    except Exception as e:
        logger.error(f"Create ScrnaClaw skill failed: {e}", exc_info=True)
        return f"Error creating ScrnaClaw skill: {e}"
...
async def execute_web_method_search(args: dict, **kwargs) -> str:
    ...
    try:
        from scrna_claw.research import search_web_markdown
...
async def execute_custom_analysis_execute(args: dict, **kwargs) -> str:
    ...
    try:
        from scrna_claw.core.capability_resolver import resolve_capability
        from scrna_claw.execution import run_autonomous_analysis
...
MAX_TOOL_ITERATIONS = int(os.getenv("SCRNA_CLAW_MAX_TOOL_ITERATIONS", "20"))  # Increased from 10, configurable
    ...
    async def after_tool(execution_result, result_record, tool_state):
        ...
        if request.executor:
            ...
            if func_name == "consult_knowledge":
                try:
                    from scrna_claw.knowledge.retriever import consume_runtime_notice
...
async def _maybe_resume_pending_preflight_request(
    *,
    ...
    pending_preflight_requests.pop(chat_id, None)
    result = await execute_scrna_claw(updated_args, session_id=session_id, chat_id=chat_id)

    pending_payload = _extract_pending_preflight_payload(result)
    if pending_payload and pending_payload.get("pending_fields"):
        pending_preflight_requests[chat_id] = {
            "tool_name": "scrna_claw",
...
• Output Directory: {OUTPUT_DIR}"""
            ...
            return f"""ℹ️ ScrnaClaw Version:

• Project: ScrnaClaw Multi-Omics Analysis Platform
...
• Repository: https://github.com/TianGzlab/ScrnaClaw

For updates and documentation, visit the GitHub repository."""

        elif cmd == "/help":
            return """# ScrnaClaw Bot Commands
...
For more info: https://github.com/TianGzlab/ScrnaClaw"""
    ...
    chat_context = await _assemble_chat_context(
        ...
        scrna_claw_dir=str(SCRNA_CLAW_DIR),
        workspace=workspace,
    ...
    hook_runtime = build_default_lifecycle_hook_runtime(SCRNA_CLAW_DIR)
