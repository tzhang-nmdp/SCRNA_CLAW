# ScrnaClaw Channels Setup Guide

This document provides detailed, step-by-step configuration guides for all supported messaging platforms in ScrnaClaw.
...
At runtime, ScrnaClaw accepts either:
- `LLM_API_KEY`
...
3. Follow prompts:
   - **Bot name**: `ScrnaClaw` (or any display name)
   - **Bot username**: `scrna_claw_bot` (must end with `_bot`, must be unique)
...
- `/setdescription @scrna_claw_bot` -> "AI research assistant for multi-omics analysis. Supports spatial transcriptomics, single-cell, genomics, proteomics, and metabolomics."
- `/setabouttext @scrna_claw_bot` -> "ScrnaClaw - Your persistent AI research partner."
- `/setcommands @scrna_claw_bot` -> 
  ```text
