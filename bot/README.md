# ScrnaClaw Messaging Bot System

The `bot` module powers ScrnaClaw's messaging-channel interfaces. It connects the core multi-omics skills engine with platforms such as Telegram, Feishu, DingTalk, Discord, Slack, WeChat, QQ, Email, and iMessage.

## Interactive Terminal Chat

Terminal chat is provided by the main ScrnaClaw CLI rather than the `bot` package:

```bash
# Start the interactive conversational UI
oc interactive
# or
scrna_claw interactive
```
...
│         ↓                                                    │
│  ┌──────────────────────────────────────────┐                │
│  │  bot/core.py  (LLM tool loop)            │───▶ scrna_claw.py (skills)
│  └──────────────────────────────────────────┘                │
└──────────────────────────────────────────────────────────────┘
```
...
ScrnaClaw bot entrypoints automatically read the project-root `.env`. If `python-dotenv` is unavailable, ScrnaClaw falls back to an internal `.env` parser, so normal `KEY=value` configuration still works.
...
| `SCRNA_CLAW_MODEL` | Override model name (optional, auto-set by provider) | All |
| `SCRNA_CLAW_LLM_TIMEOUT_SECONDS` | Total timeout for shared LLM requests in seconds (default: `120`) | All |
| `SCRNA_CLAW_LLM_CONNECT_TIMEOUT_SECONDS` | Connection timeout for shared LLM requests in seconds (default: `10`) | All |
...
| `SCRNA_CLAW_DATA_DIRS` | Extra trusted data directories (comma-separated absolute paths) | All |
| `SCRNA_CLAW_MEMORY_DB_URL` | Persistent graph memory database URL | All |
| `SCRNA_CLAW_MAX_HISTORY` | Max messages kept in transcript history (default: 50) | All |
| `SCRNA_CLAW_MAX_HISTORY_CHARS` | Optional transcript character cap (default: 0 = disabled) | All |
| `SCRNA_CLAW_MAX_TOOL_ITERATIONS` | Max tool iterations per request (default: 20) | All |
...
ScrnaClaw supports **12 LLM providers** through a unified OpenAI-compatible interface:
...
| Custom endpoint | `custom` | *(set `SCRNA_CLAW_MODEL`)* | *(set `LLM_API_KEY`)* |
...
SCRNA_CLAW_MODEL=qwen-max

# Local Ollama
LLM_PROVIDER=ollama
SCRNA_CLAW_MODEL=qwen2.5:7b

# Custom OpenAI-compatible endpoint
LLM_PROVIDER=custom
LLM_BASE_URL=https://your-endpoint.example.com/v1
SCRNA_CLAW_MODEL=your-model-name
...
SCRNA_CLAW_MODEL=deepseek-reasoner   # Use R1 instead of default V3
```
...
| `/skills` | List all available ScrnaClaw analysis skills |
...
Bot:  (reads from NAS if SCRNA_CLAW_DATA_DIRS includes /mnt/nas/spatial)
```
...
- Any additional paths in `SCRNA_CLAW_DATA_DIRS`

To add external data directories (NAS, shared storage, other projects), set in `.env`:

```bash
SCRNA_CLAW_DATA_DIRS=/mnt/nas/spatial_data,/home/user/experiments
```
...
- File paths are validated against a trusted directory whitelist (`data/`, `examples/`, `output/`, `SCRNA_CLAW_DATA_DIRS`)
...
This is a known issue with `lark_oapi` — it uses a module-level event loop variable. ScrnaClaw patches this automatically when running via `python -m bot.run`. If you see this error, ensure you're using the latest version of `bot/channels/feishu.py`.
