"""
bot/onboard.py — Interactive Setup Wizard for ScrnaClaw
"""
...
def _configure_llm(env_vars: dict[str, str | None]) -> tuple[bool, str]:
    ...
    current_model = str(env_vars.get("SCRNA_CLAW_MODEL", "") or "")
    model_default = current_model if current_provider == provider and current_model else preset_model
    if not _prompt_field(
        env_vars,
        FieldSpec("SCRNA_CLAW_MODEL", "Model name", required=True, default=model_default),
    ):
        return False, ""
    ...
    configure_timeouts = _ask_confirm(
        ...
        default=bool(
            env_vars.get("SCRNA_CLAW_LLM_TIMEOUT_SECONDS")
            or env_vars.get("SCRNA_CLAW_LLM_CONNECT_TIMEOUT_SECONDS")
        ),
    )
    ...
    if configure_timeouts and not _prompt_fields(
        ...
        (
            ...
            FieldSpec(
                "SCRNA_CLAW_LLM_CONNECT_TIMEOUT_SECONDS",
...
def _configure_core_runtime(env_vars: dict[str, str | None]) -> bool:
    ...
    configure_memory = _ask_confirm(
        ...
        default=bool(
            env_vars.get("SCRNA_CLAW_MEMORY_DB_URL")
            or env_vars.get("SCRNA_CLAW_MEMORY_API_TOKEN")
            or env_vars.get("SCRNA_CLAW_MEMORY_HOST")
            or env_vars.get("SCRNA_CLAW_MEMORY_PORT")
        ),
    )
    ...
    if configure_memory and not _prompt_fields(
        ...
        (
            FieldSpec(
                ...
                "Memory database URL",
                default="sqlite+aiosqlite:///~/.config/scrna_claw/memory.db",
            ),
            FieldSpec("SCRNA_CLAW_MEMORY_API_TOKEN", "Memory API bearer token", secret=True),
            FieldSpec("SCRNA_CLAW_MEMORY_HOST", "Memory API host", default="127.0.0.1"),
            FieldSpec("SCRNA_CLAW_MEMORY_PORT", "Memory API port", default="8766"),
        ),
    ):
        return False
    ...
    configure_runtime_limits = _ask_confirm(
        ...
        default=bool(
            env_vars.get("SCRNA_CLAW_MAX_HISTORY")
            or env_vars.get("SCRNA_CLAW_MAX_TOOL_ITERATIONS")
        ),
    )
    ...
    if configure_runtime_limits and not _prompt_fields(
        env_vars,
        (
            FieldSpec("SCRNA_CLAW_MAX_HISTORY", "Max history messages kept in memory", default="50"),
            FieldSpec(
...
def run_onboard() -> None:
    console.print(
        Panel.fit(
            "[bold cyan]ScrnaClaw Configuration Wizard[/bold cyan]\n"
