# AGENTS.md — ScrnaClaw Guide for AI Coding Agents

This guide is for AI coding agents working on the ScrnaClaw codebase.

## Project Overview

ScrnaClaw is a multi-omics analysis platform supporting 5 domains: spatial transcriptomics, single-cell omics, genomics, proteomics, and metabolomics. Each skill is a self-contained module that performs a specific analysis task via CLI or Python API. All processing is local-first. Design is inspired by [ClawBio](https://github.com/ClawBio/ClawBio).

**Note**: ScrnaClaw evolved from SpatialClaw and now uses a unified `scrna_claw.py` entrypoint.

## Setup

```bash
cd /data1/TianLab/zhouwg/project/ScrnaClaw
pip install -e .
...
python scrna_claw.py list   # or: sc list
python scrna_claw.py run spatial-preprocess --demo
```

> **`sc` short alias**: After `pip install -e .`, both `scrna_claw` and `sc` commands
> are available system-wide. `sc` is registered via `[project.scripts]` in
> `pyproject.toml` and points to `scrna_claw.cli:main` — the same entry point as
> `scrna_claw`. No PATH tricks needed.
>
...
> Both `python scrna_claw.py <cmd>` and the short alias `sc <cmd>` work identically
> after `pip install -e .` (or `make install-sc`).

| Command | Purpose |
|---------|---------|
| `sc list` | List all 50+ skills across 5 domains |
| `sc run <skill> --demo` | Run a skill with demo data |
| `sc run <skill> --input <file> --output <dir>` | Run with user data |
| `sc interactive` | **Start interactive terminal chat (CLI mode)** |
| `sc interactive --ui tui` | **Start full-screen Textual TUI** |
| `sc interactive -p "<prompt>"` | **Single-shot mode (non-interactive)** |
| `sc interactive --session <id>` | **Resume a previous session** |
| `sc tui` | Alias for `interactive --ui tui` |
| `sc mcp list` | List configured MCP servers |
| `sc mcp add <name> <cmd> [args]` | Add an MCP server |
| `sc mcp remove <name>` | Remove an MCP server |
| `sc mcp config` | Show MCP config file path |
| `sc onboard` | Run interactive setup wizard for LLM, runtime, memory, and channels |
| `python -m pytest -v` | Run all tests |
| `make test` | Alias for pytest |
| `make demo` | Run preprocess demo |
| `make install-sc` | (Re)install package + activate `sc` alias |
| `make oc-link` | Quick wrapper script in `~/.local/bin/sc` (no pip) |
...
```
ScrnaClaw/
├── scrna_claw.py                # Main CLI runner (SKILLS dict, DOMAINS registry)
├── scrna_claw/                  # Core framework (domain-agnostic)
...
> `scrna_claw/` contains only domain-agnostic framework code (core, loaders,
...
Skills are registered in `scrna_claw/core/registry.py` and dynamically discovered from `skills/`.
...
6. Register stable aliases in `scrna_claw/core/registry.py` (or rely on dynamic discovery)
7. Add test path to `pytest.ini`
8. Regenerate catalog: `python scripts/generate_catalog.py`

## Graph Memory System

ScrnaClaw uses a centralized graph-based memory system to persist context across sessions, agents, and tool invocations. The core system is located in `scrna_claw/memory/`.
...
- **MemoryClient**: High-level abstract client (`scrna_claw/memory/memory_client.py`) used by multi-agent pipelines to `remember()`, `recall()`, and `search()`.
- **Compat Layer**: To maintain backward compatibility with old bot memory, `scrna_claw/memory/compat.py` implements the old interface but routes it through the graph engine.
- **REST API**: A FastAPI backend provides management capabilities, accessible via `sc memory-server`.
...
The memory API binds to `127.0.0.1:8766` by default. If you bind it to a non-local interface, set `SCRNA_CLAW_MEMORY_API_TOKEN` as well.

### Configuration (Environment Variables)

- `SCRNA_CLAW_MEMORY_DB_URL`: SQLAlchemy connection URL (`sqlite+aiosqlite:///bot/data/memory.db`)
- `SCRNA_CLAW_MEMORY_API_TOKEN`: Bearer token required when exposing the API beyond localhost.

## Bot Integration

ScrnaClaw includes multi-channel messaging frontends in `bot/`:
...
- TOOLS definition (scrna_claw, save_file, write_file, generate_audio)
- `execute_scrna_claw()` — runs `scrna_claw.py run <skill>` as subprocess
- Security helpers (path sanitization, file size limits)
- Audit logging (JSONL)

The persona is defined in `SOUL.md` (OmicsBot, the ScrnaClaw AI assistant).
...
ScrnaClaw includes multi-channel bot frontends in `bot/`. They all import `bot/core.py`, which provides the shared LLM tool-use loop, skill execution, security helpers, and audit logging. Each frontend handles platform-specific message handling, media upload/download, and rate limiting.
...
2. **Disclaimer required**: Every report must include the ScrnaClaw disclaimer
3. **No hallucinated science**: All parameters trace to SKILL.md or cited tools
4. **Security filtering**: `scrna_claw.py` enforces `allowed_extra_flags` whitelists

## Interactive CLI/TUI

ScrnaClaw features a full interactive terminal interface (referencing EvoScientist's architecture).

### Architecture

```
scrna_claw.py interactive
    └── scrna_claw/interactive/interactive.py   # prompt_toolkit REPL
...
scrna_claw.py tui  (or --ui tui)
    └── scrna_claw/interactive/tui.py           # Textual full-screen TUI
```
...
sc tui
sc interactive --ui tui

# Single-shot (non-interactive)
sc interactive -p "run spatial-preprocessing demo"

# Resume a previous session
sc interactive --session <session-id>

# Override model/provider
sc interactive --provider deepseek --model deepseek-chat

# Set working directory
sc interactive --workspace /path/to/workdir

# Daemon mode (persistent workspace, default behavior)
sc interactive --mode daemon

# Run mode (isolated per-session workspace)
sc interactive --mode run

# Run mode with a named workspace
sc interactive --mode run --name my-analysis
```
...
sc mcp add sequential-thinking npx -- -y @modelcontextprotocol/server-sequential-thinking

# Add an HTTP-based MCP server
sc mcp add my-server http://localhost:8080

# List all configured MCP servers
sc mcp list

# Remove an MCP server
sc mcp remove sequential-thinking

# Show config file location
sc mcp config
# → ~/.config/scrna_claw/mcp.yaml
```

MCP tools are loaded from `~/.config/scrna_claw/mcp.yaml` at session start.
...
Sessions are saved to `~/.config/scrna_claw/sessions.db` (SQLite).
