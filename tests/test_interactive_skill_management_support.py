...
from scrna_claw.interactive._skill_management_support import (
    SkillCommandStatus,
    SkillEnablementPlan,
    SkillInstallPlan,
    InstalledSkillEntry,
...
def test_prepare_skill_install_plan_rejects_empty_source(tmp_path):
    result = prepare_skill_install_plan("", scrna_claw_dir=tmp_path)
...
def test_finalize_installed_skill_reports_validation_and_refresh(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "scrna_claw.interactive._skill_management_support.refresh_skill_registry",
        lambda: "",
    )
    ...
    assert (dest / ".scrna_claw-install.json").exists()
    assert (dest / ".scrna_claw-extension-state.json").exists()
...
def test_install_extension_from_source_installs_local_prompt_pack(monkeypatch, tmp_path):
    ...
    (source / "scrna_claw-extension.json").write_text(
        json.dumps(
            {
                "name": "my-prompts",
                "version": "1.0.0",
    ...
    statuses = install_extension_from_source(str(source), scrna_claw_dir=tmp_path)
...
def test_install_extension_from_source_rejects_untrusted_prompt_pack(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "scrna_claw.interactive._skill_management_support._stage_github_source",
        lambda _plan, _staging_root: (source, SkillCommandStatus("success", "cloned")),
    )

    statuses = install_extension_from_source("https://github.com/user/my-prompts", scrna_claw_dir=tmp_path)
...
def test_prepare_extension_uninstall_plan_identifies_removable_extension(tmp_path):
    ...
    (candidate / ".scrna_claw-install.json").write_text(
        (
    ...
    result = prepare_extension_uninstall_plan("my-prompts", scrna_claw_dir=tmp_path)
...
def test_prepare_skill_uninstall_plan_reports_builtin_skill(tmp_path):
    ...
    result = prepare_skill_uninstall_plan("my-skill", scrna_claw_dir=tmp_path)
...
def test_prepare_extension_enablement_plan_and_finalize(monkeypatch, tmp_path):
    ...
    (skill_dir / ".scrna_claw-install.json").write_text(
        (
    ...
    (skill_dir / ".scrna_claw-extension-state.json").write_text(
...
def test_build_installed_extension_list_view_marks_tracked_and_disabled_entries(tmp_path):
    ...
    (tracked / "scrna_claw-extension.json").write_text(
        json.dumps(
            {
    ...
    (tracked / ".scrna_claw-install.json").write_text(
        (
    ...
    (tracked / ".scrna_claw-extension-state.json").write_text(
        '{"enabled": false, "updated_at": "2026-04-01T00:00:00+00:00", "disabled_reason": "manual"}',
        encoding="utf-8",
    )
    ...
    view = build_installed_extension_list_view(scrna_claw_dir=tmp_path)
...
def test_build_installed_skill_list_view_filters_to_skill_packs(tmp_path):
    ...
    (prompt_pack / ".scrna_claw-install.json").write_text(
        (
            '{"extension_name":"tracked-prompts","source_kind":"local","source":"/tmp/prompts",'
    ...
    view = build_installed_skill_list_view(scrna_claw_dir=tmp_path)
...
def test_build_refresh_extensions_statuses_reports_inventory(monkeypatch, tmp_path):
    ...
    (user_skill / ".scrna_claw-install.json").write_text(
        (
            '{"extension_name":"tracked-skill","source_kind":"local","source":"/tmp/tracked-skill",'
            '"installed_at":"2026-04-01T00:00:00+00:00","extension_type":"skill-pack"}'
        ),
    ...
    statuses = build_refresh_extensions_statuses(scrna_claw_dir=tmp_path)
...
def test_install_extension_from_source_tracks_hook_pack_as_disabled(monkeypatch, tmp_path):
    ...
    state = json.loads((installed / ".scrna_claw-extension-state.json").read_text(encoding="utf-8"))
...
def test_build_refresh_skills_statuses_reports_detected_user_packs(monkeypatch, tmp_path):
    ...
    statuses = build_refresh_skills_statuses(scrna_claw_dir=tmp_path)
