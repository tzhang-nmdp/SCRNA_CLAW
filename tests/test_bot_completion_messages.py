...
from scrna_claw.core.skill_scaffolder import SkillScaffoldResult
from scrna_claw.knowledge.retriever import _push_runtime_notice, clear_runtime_notices
from scrna_claw.runtime.policy import TOOL_POLICY_REQUIRE_APPROVAL, ToolPolicyDecision
...
async def test_execute_create_omics_skill_includes_gate_summary(monkeypatch):
    monkeypatch.setattr(
        "scrna_claw.core.skill_scaffolder.create_skill_scaffold",
    ...
    assert "Created ScrnaClaw skill scaffold." in message
...
async def test_execute_custom_analysis_execute_includes_gate_summary(monkeypatch):
    monkeypatch.setattr(
        "scrna_claw.core.capability_resolver.resolve_capability",
        lambda *_args, **_kwargs: _FakeCapabilityDecision(),
    )
    monkeypatch.setattr(
        "scrna_claw.execution.run_autonomous_analysis",
        lambda **_kwargs: {
            "ok": True,
