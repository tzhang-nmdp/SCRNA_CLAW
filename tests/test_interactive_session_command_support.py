...
from scrna_claw.interactive._session_command_support import (
...
async def test_build_session_list_view_uses_session_rows(monkeypatch):
    ...
    monkeypatch.setattr(
        "scrna_claw.interactive._session_command_support.list_sessions",
        _list_sessions,
    )
    monkeypatch.setattr(
        "scrna_claw.interactive._session_command_support.format_relative_time",
        lambda _: "2h ago",
    )
...
def test_build_export_session_command_view_exports_markdown(tmp_path):
    ...
    assert view.export_path == str(tmp_path / "exports" / "scrna_claw_session_abc12345.md")
...
def test_build_export_session_command_view_reports_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "scrna_claw.interactive._session_command_support.export_conversation_to_markdown",
        _fail,
    )
...
def test_enrich_session_metadata_derives_runtime_state(tmp_path, monkeypatch):
    monkeypatch.setattr(
        "scrna_claw.interactive._session_command_support.resolve_pipeline_workspace",
        lambda _arg, workspace: Path(workspace),
    )
    monkeypatch.setattr(
        "scrna_claw.interactive._session_command_support.load_pipeline_workspace_snapshot",
    ...
    monkeypatch.setattr(
        "scrna_claw.interactive._session_command_support.list_installed_extensions",
        lambda _root: [
            SimpleNamespace(
...
async def test_build_resume_session_command_view_loads_and_formats_history(monkeypatch):
    ...
    monkeypatch.setattr(
        "scrna_claw.interactive._session_command_support.load_session",
        _load_session,
    )
...
def test_build_current_session_command_view_includes_pipeline_snapshot(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "scrna_claw.interactive._session_command_support.build_pipeline_display_from_snapshot",
...
async def test_build_delete_session_command_view_deletes_loaded_session(monkeypatch):
    ...
    monkeypatch.setattr(
        "scrna_claw.interactive._session_command_support.delete_session",
        _delete_session,
    )
