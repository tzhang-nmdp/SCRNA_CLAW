...
def test_is_local_bind_host():
    from scrna_claw.memory.server import _is_local_bind_host
...
def test_validate_server_security_rejects_remote_without_token():
    from scrna_claw.memory.server import _validate_server_security

    with pytest.raises(SystemExit, match="SCRNA_CLAW_MEMORY_API_TOKEN"):
...
def test_memory_server_docs_and_openapi_require_auth_when_token_set(monkeypatch, tmp_path):
    ...
    import scrna_claw.memory as memory
    from scrna_claw.memory.server import _build_app

    monkeypatch.setenv(
        "SCRNA_CLAW_MEMORY_DB_URL",
        f"sqlite+aiosqlite:///{tmp_path / 'memory-security.db'}",
    )
    monkeypatch.setenv("SCRNA_CLAW_MEMORY_API_TOKEN", "secret-token")
