...
from scrna_claw.interactive import _mcp
from scrna_claw.extensions import write_extension_state, write_install_record
...
def test_assemble_chat_context_forwards_knowledge_guidance_to_prompt_builder(monkeypatch):
    ...
    monkeypatch.setattr(
        "scrna_claw.runtime.context_layers.load_knowledge_guidance",
...
def test_assemble_prompt_context_includes_active_prompt_pack_layer(tmp_path):
    ...
    (prompt_pack / "scrna_claw-extension.json").write_text(
        json.dumps(
            {
                "name": "local-rules",
                "version": "1.0.0",
...
def test_assemble_chat_context_forwards_prompt_pack_context_to_builder(tmp_path):
    ...
    assert context.system_prompt == "PROMPT"
    assert calls["prompt"]["scrna_claw_dir"] == str(tmp_path)
