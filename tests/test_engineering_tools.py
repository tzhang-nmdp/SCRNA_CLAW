...
from scrna_claw.runtime.bot_tools import BotToolContext, build_bot_tool_specs
from scrna_claw.runtime.engineering_tools import (
...
def test_scrna_claw_tool_description_mentions_sc_batch_auto_prepare():
    specs = build_bot_tool_specs(
        BotToolContext(
    ...
    omics_spec = next(spec for spec in specs if spec.name == "scrna_claw")
            ...
            executors["task_update"](
                {
                    ...
                    "artifact_ref": "scrna_claw/runtime/engineering_tools.py",
                },
                session_id="interactive:user:chat-1",
                chat_id="chat-1",
                surface="interactive",
...
def test_web_fetch_uses_shared_fetcher_and_truncates(monkeypatch, tmp_path: Path):
    executors = _build_executors(tmp_path)
    import scrna_claw.research.web_search as web_search
...
def test_mcp_list_sanitizes_environment_values(monkeypatch, tmp_path: Path):
    ...
    (config_root / "scrna_claw").mkdir(parents=True, exist_ok=True)
    (config_root / "scrna_claw" / "mcp.yaml").write_text(
        "demo:\n"
        "  transport: stdio\n"
        "  command: npx\n"
        "  args:\n"
