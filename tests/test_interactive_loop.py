...
async def test_stream_llm_response_uses_explicit_workspace_context(monkeypatch):
    ...
    async def _fake_llm_tool_loop(
        ...
        *,
        ...
        core_module.conversations[conversation_id] = [
            ...
            {"role": "assistant", "content": "I am ScrnaClaw."},
        ]
        if on_stream_content is not None:
            await on_stream_content("I am ")
            await on_stream_content("ScrnaClaw.")
        return "I am ScrnaClaw."
    ...
    assert rendered.count("I am ScrnaClaw.") == 1
    assert messages == [
...
def test_init_llm_does_not_force_registry_load(monkeypatch):
    ...
    for name in (
        "bot.core",
        "scrna_claw.runtime.context_assembler",
        "scrna_claw.runtime.context_layers",
    ):
    ...
    monkeypatch.setenv("SCRNA_CLAW_MODEL", "test-model")
