...
def test_onboard_save_env_updates_and_removes_keys(monkeypatch, tmp_path):
    ...
    env_path.write_text(
        "\n".join(
            [
                ...
                "SCRNA_CLAW_DATA_DIRS='/mnt/data one'",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    assert parsed["SCRNA_CLAW_DATA_DIRS"] == "/mnt/data one"
...
def test_run_onboard_writes_core_runtime_and_channel_config(monkeypatch, tmp_path):
    ...
    assert "SCRNA_CLAW_MODEL=deepseek-chat" in saved
    assert "LLM_API_KEY=sk-test" in saved
    assert "SCRNA_CLAW_LLM_TIMEOUT_SECONDS=90" in saved
    assert "SCRNA_CLAW_LLM_CONNECT_TIMEOUT_SECONDS=12" in saved
    assert "RATE_LIMIT_PER_HOUR=15" in saved
    assert "SCRNA_CLAW_DATA_DIRS=/mnt/nas,/srv/data" in saved
    assert "ALLOWED_SENDERS=user-a,user-b" in saved
    assert "GLOBAL_RATE_LIMIT=180" in saved
    assert "SCRNA_CLAW_MEMORY_DB_URL=sqlite+aiosqlite:///tmp/omics.db" in saved
    assert "SCRNA_CLAW_MEMORY_API_TOKEN=memory-secret" in saved
    assert "SCRNA_CLAW_MAX_HISTORY=60" in saved
    assert "SCRNA_CLAW_MAX_HISTORY_CHARS=5000" in saved
    assert "SCRNA_CLAW_MAX_TOOL_ITERATIONS=25" in saved
