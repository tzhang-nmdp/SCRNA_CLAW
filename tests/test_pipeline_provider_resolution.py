...
def test_pipeline_delegates_llm_construction_to_provider_factory(monkeypatch):
    ...
    monkeypatch.setattr(
        "scrna_claw.agents.pipeline.get_langchain_llm",
        _fake_factory,
    )
    monkeypatch.setenv("SCRNA_CLAW_LLM_TIMEOUT_SECONDS", "33")
    monkeypatch.setenv("SCRNA_CLAW_LLM_CONNECT_TIMEOUT_SECONDS", "6")
    monkeypatch.setenv("LLM_BASE_URL", "https://gateway.example.test/v1")
