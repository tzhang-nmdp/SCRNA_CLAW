...
from scrna_claw.execution import validate_custom_analysis_code
...
def test_resolve_capability_partial_skill():
    decision = resolve_capability(
        "Run spatial preprocessing and then compute a custom neighborhood entropy score not in ScrnaClaw"
    )
...
def test_resolve_capability_marks_skill_creation_requests():
    decision = resolve_capability(
        "Create a new ScrnaClaw skill for CellCharter-based spatial domain analysis"
    )
