...
def experimenthub_cache_dir() -> str:
    ...
    return os.getenv("SCRNA_CLAW_EXPERIMENTHUB_CACHE", str(Path.home() / ".cache" / "scrna_claw" / "experimenthub"))
...
def build_celltypist_input_adata(adata: AnnData):
    ...
    ScrnaClaw annotation workflows prefer normalized expression in ``adata.X``.
...
def _apply_popv_annotation_official(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="scrna_claw_popv_") as tmpdir:
        proc = Process_Query(
