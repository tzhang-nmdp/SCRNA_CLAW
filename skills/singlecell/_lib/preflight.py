...
from scrna_claw.common.user_guidance import emit_user_guidance, emit_user_guidance_payload
    ...
    *,
    ...
    if method == "markers":
        ...
        if not _declared_x_is_normalized(adata) and matrix_looks_count_like(adata.X):
            decision.require_confirmation(
                "`markers` annotation expects normalized expression. Run `sc-preprocessing` first or confirm that the current matrix has already been transformed outside ScrnaClaw."
            )
        decision.add_guidance(
    ...
    *,
    ...
    if not _looks_preprocessed_for_integration(adata):
        decision.add_guidance(
            ...
            "The standard ScrnaClaw path is canonicalize the input first, then `sc-preprocessing`, then `sc-batch-integration`."
        )
