...
INPUT_CONTRACT_KEY = "scrna_claw_input_contract"
INPUT_CONTRACT_VERSION = "1.0"
MATRIX_CONTRACT_KEY = "scrna_claw_matrix_contract"
...
def record_standardized_input_contract(
    ...
    *,
    ...
    """Persist a minimal canonical ScrnaClaw single-cell input contract."""
...
def build_standardization_recommendation(
    *,
    ...
    return (
        f"{prefix} input that has not yet been canonicalized under the ScrnaClaw scRNA contract. "
...
def prepare_count_like_adata(
    ...
    *,
    ...
    prepared.var["_scrna_claw_qc_gene_name"] = best_names.astype(str)
    if gene_name_source != "var_names":
        prepared.var["_scrna_claw_original_var_names"] = prepared.var_names.astype(str)
...
def canonicalize_singlecell_adata(
    ...
    *,
    ...
    The returned object is safe for downstream ScrnaClaw scRNA skills: ``X`` is
    ...
    if "_scrna_claw_original_var_names" in standardized.var.columns and "feature_id" not in standardized.var.columns:
        standardized.var["feature_id"] = standardized.var["_scrna_claw_original_var_names"].astype(str)
...
def require_preprocessed(adata: AnnData) -> None:
    ...
    if "X_pca" not in adata.obsm:
        raise PreprocessingRequiredError(
            ...
            "  python scrna_claw.py run sc-preprocess --input data.h5ad --output results/"
        )
...
def store_analysis_metadata(adata: AnnData, skill_name: str, method: str, params: dict) -> None:
    ...
    analyses = adata.uns.get("scrna_claw_analyses")
    if not isinstance(analyses, dict):
        analyses = {}
        adata.uns["scrna_claw_analyses"] = analyses

    analyses[str(skill_name)] = record
    adata.uns[f"scrna_claw_{skill_name}"] = record
