...
def _resolve_cellphonedb_database() -> Path:
    ...
    cache_dir = Path.home() / ".cache" / "scrna_claw" / "cellphonedb" / CELLPHONEDB_DB_VERSION
...
def run_cellchat(adata, *, cell_type_key: str, species: str) -> pd.DataFrame:
    ...
    export, source = _build_cellchat_input_adata(adata)
    with tempfile.TemporaryDirectory(prefix="scrna_claw_cellchat_") as tmpdir:
        tmpdir = Path(tmpdir)
...
def run_nichenet(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="scrna_claw_nichenet_") as tmpdir:
        tmpdir = Path(tmpdir)
        input_h5ad = tmpdir / "input.h5ad"
        output_dir = tmpdir / "output"
        output_dir.mkdir(parents=True, exist_ok=True)
...
def run_cellphonedb(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="scrna_claw_cellphonedb_") as tmpdir:
...
def run_communication(
    ...
    *,
    ...
    if method == "cellphonedb":
        summary["score_semantics"] = (
            "CellPhoneDB score comes from the official statistical-analysis output reshaped into the ScrnaClaw contract."
        )
