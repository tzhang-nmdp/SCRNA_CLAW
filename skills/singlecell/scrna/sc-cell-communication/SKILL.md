    ...
    param_hints:
      ...
      cellphonedb:
        ...
        tips:
          ...
          - "--cellphonedb-threshold and --cellphonedb-iterations are the main public CellPhoneDB tuning knobs in ScrnaClaw."
      cellchat_r:
        priority: "cell_type_key -> species"
        params: ["cell_type_key", "species"]
        defaults: {cell_type_key: "cell_type", species: "human"}
...
- Why ScrnaClaw: one output contract captures interactions, top pairs, and summary figures.
...
```bash
python scrna_claw.py run sc-cell-communication \
...
Current ScrnaClaw built-in communication path:
...
Current ScrnaClaw CellPhoneDB path:
...
3. normalizes CellPhoneDB outputs back into the standard ScrnaClaw interaction contract

### `cellchat_r`

Current ScrnaClaw CellChat path:
...
Current ScrnaClaw NicheNet path:
...
- the grouping column is the main scientific control in this wrapper; the current ScrnaClaw surface does not expose the full LIANA or CellChat parameter space
