    ...
    param_hints:
      ...
      starsolo:
        ...
        tips:
          ...
          - "--whitelist: strongly recommended; ScrnaClaw only auto-detects common local v2/v3 whitelist files."
...
You are **SC Count**, a specialized ScrnaClaw agent for converting scRNA-seq
...
- **Without it**: users must run Cell Ranger or STARsolo manually, then convert
  outputs again before starting ScrnaClaw downstream skills.
- **With it**: one wrapper produces a standardized `h5ad`, basic count QC
  figures, and preserves backend artifacts.
- **Why ScrnaClaw**: it narrows the public surface to mainstream 10x-oriented
...
3. **Stable downstream contract**: writes `processed.h5ad` with
   `layers['counts']` and ScrnaClaw input-contract metadata.
...
4. **Persist results**: load the filtered matrix, standardize it into the ScrnaClaw AnnData contract, and preserve backend artifact paths.
...
- "用 STARsolo 产出能接 ScrnaClaw 的 adata"
...
4. **Standardize output**: create `layers['counts']`, stabilize names, and record ScrnaClaw input-contract metadata.
...
4. **Standardize output**: create `layers['counts']` and record the same ScrnaClaw contract as the Cell Ranger path.
...
4. **Standardize output**: create the same ScrnaClaw count handoff contract.
...
> **Current ScrnaClaw behavior**: all non-Cell Ranger methods are intentionally
...
- The filtered count matrix is always converted into the ScrnaClaw AnnData
  input contract.
...
- External tools that ScrnaClaw does **not** auto-install:
  - `cellranger`
  - `STAR`
  - `simpleaf`
  - `kb`
...
- Reference files are also user-managed resources; ScrnaClaw should guide users to `resources/singlecell/references/...` or accept explicit local paths, but should not silently download large references during execution.
