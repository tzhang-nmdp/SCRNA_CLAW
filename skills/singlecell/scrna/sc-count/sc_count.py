...
def main() -> None:
    ...
    if args.demo:
        ...
        if artifacts is None and not args.demo:
            if reference_path is None:
                raise ValueError(
                    ...
                    f"Pass `--reference /abs/path/...`, or keep one under `{_recommended_reference_dir(args.method)}` so ScrnaClaw can auto-detect it.\n\n"
        ...
        if args.method in {"simpleaf", "kb_python"}:
            standardized.uns["scrna_claw_count_artifacts"] = {
    ...
    result_data = {
        ...
        "matrix_contract": standardized.uns.get("scrna_claw_matrix_contract", {}),
        ...
        "artifacts": standardized.uns.get("scrna_claw_count_artifacts", {}),
        "output_files": {
