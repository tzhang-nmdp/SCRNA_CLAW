...
def run_soupx(raw_matrix_dir: str, filtered_matrix_dir: str):
    ...
    with tempfile.TemporaryDirectory(prefix="scrna_claw_soupx_") as tmpdir:
        tmpdir = Path(tmpdir)
...
def write_ambient_report(output_dir: Path, summary: dict, params: dict, input_file: str | None) -> None:
    ...
    body_lines.extend([
        "",
        "## Outputs\n",
        f"- **ScrnaClaw downstream export**: `{output_bundle.get('corrected_h5ad', 'corrected.h5ad')}`",
    ...
    if method_outputs:
        ...
        if executed_method == "cellbender":
            body_lines.append("- `corrected.h5ad` is ScrnaClaw's wrapped AnnData export for downstream skills.")
    ...
    body_lines.extend([
        ...
        "ScrnaClaw preserves the original CellBender matrix outputs and additionally writes `corrected.h5ad` for downstream interoperability.",
        "",
        "### SoupX (R)",
