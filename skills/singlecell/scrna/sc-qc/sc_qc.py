...
def _build_qc_visualization_recipe(_adata, _summary: dict, _context: dict) -> VisualizationRecipe:
    ...
    return VisualizationRecipe(
        ...
        description="Default ScrnaClaw QC diagnostic gallery for scRNA-seq quality assessment.",
        plots=[
            PlotSpec(
                plot_id="qc_violin",
                role="overview",
...
def write_qc_report(output_dir: Path, summary: dict, effective_params: dict, input_file: str | None, *, gallery_context: dict | None = None) -> None:
    ...
    body_lines = [
        ...
        "- **Output object standardization**: Yes; `processed.h5ad` is saved in the ScrnaClaw scRNA contract for downstream reuse.",
    ...
    body_lines.extend([
        ...
        "- `processed.h5ad` — AnnData object with QC metrics added to `.obs`, feature tags added to `.var`, and ScrnaClaw analysis metadata added to `.uns`",
        "",
    ])
...
def main():
    ...
    logger.info("Canonicalizing input into the ScrnaClaw scRNA contract...")
    ...
    processed_adata.uns["scrna_claw_matrix_contract"] = matrix_contract
