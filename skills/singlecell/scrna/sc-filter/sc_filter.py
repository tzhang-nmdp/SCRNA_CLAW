...
def _build_filter_visualization_recipe() -> VisualizationRecipe:
    ...
    return VisualizationRecipe(
        ...
        description="Default ScrnaClaw gallery for threshold-based single-cell filtering.",
        plots=plots,
    )
...
def main():
    ...
    if had_qc_metrics and original_x_kind == "normalized_expression":
        ...
        input_contract = ensure_input_contract(
            working_adata,
            source_path=input_file,
            standardized=bool(working_adata.uns.get("scrna_claw_input_contract", {}).get("standardized", False)),
        )
        prepared_input = None
    else:
