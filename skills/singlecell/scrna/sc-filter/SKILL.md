  ...
  scrna_claw:
    ...
    param_hints:
      threshold_filtering:
        ...
        tips:
          - "--tissue: Wrapper-level preset that overrides the default QC thresholds with ScrnaClaw tissue heuristics."
...
- Why ScrnaClaw: tissue presets and a stable output contract make filtering easier to reproduce.
...
| `--tissue` | none | ScrnaClaw preset thresholds such as `pbmc`, `brain`, or `tumor` |

## Algorithm / Methodology

Current ScrnaClaw `threshold_filtering` always:
...
- `tissue` is an ScrnaClaw wrapper preset, not an upstream Scanpy parameter.
...
- Threshold presets are ScrnaClaw wrapper defaults, not upstream standard recommendations for every tissue.
