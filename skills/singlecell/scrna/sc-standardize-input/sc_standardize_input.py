#!/usr/bin/env python3
"""Standardize single-cell AnnData input into the ScrnaClaw canonical contract."""
...
def _write_report(output_dir: Path, summary: dict, input_file: str | None) -> None:
    ...
    lines = [
        ...
        "",
        "## Input Contract\n",
        "- `adata.X`: raw count-like matrix for downstream ScrnaClaw skills that need counts",
        "- `adata.layers['counts']`: canonical raw counts copy",
        "- `adata.raw`: count-like snapshot aligned to the standardized object",
        "- `adata.var_names`: standardized feature names used by ScrnaClaw",
        "- `adata.var['gene_symbols']`: user-facing gene symbols when available",
        "- `adata.uns['scrna_claw_input_contract']`: provenance and standardization metadata",
        "- `adata.uns['scrna_claw_matrix_contract']`: explicit matrix semantics for `X`, `raw`, and `layers`",
        "",
        "## Warnings\n",
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Standardize single-cell input into the ScrnaClaw canonical AnnData contract")
    ...
    standardized.uns["scrna_claw_matrix_contract"] = matrix_contract
    ...
    write_output_readme(
        ...
        description="Canonicalize single-cell input into a stable AnnData contract for downstream ScrnaClaw skills.",
