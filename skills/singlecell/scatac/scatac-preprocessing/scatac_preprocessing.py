...
_NUMBA_CACHE_DIR = Path(tempfile.gettempdir()) / "scrna_claw-numba-cache"
_MPLCONFIGDIR = Path(tempfile.gettempdir()) / "scrna_claw-mplconfig"
...
def _build_scatac_visualization_recipe(adata, summary: dict, context: dict) -> VisualizationRecipe:
    ...
    return VisualizationRecipe(
        ...
        description=f"Default ScrnaClaw scATAC preprocessing gallery for method '{summary.get('method', '')}'.",
        plots=plots,
    )
    ...
    *,
    ...
    body_lines.extend(
        [
            ...
            "- `figures/` — standard ScrnaClaw scATAC preprocessing gallery.",
