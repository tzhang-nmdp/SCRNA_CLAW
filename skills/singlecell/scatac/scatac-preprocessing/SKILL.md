...
description: >-
  ...
  clustering, then exports a downstream-ready AnnData plus a standard ScrnaClaw
...
You are **scATAC Preprocessing**, the ScrnaClaw skill for turning raw-count-like
...
embedding, neighborhood graph, UMAP, clustering, and a standard ScrnaClaw
result bundle.
...
- **With it**: one command produces a stable scATAC preprocessing object plus a
  standard ScrnaClaw report, gallery, tables, and reproducibility bundle
- **Why ScrnaClaw**: the wrapper keeps a compact, implementation-aligned
  contract around a Signac-style TF-IDF + LSI workflow while exposing only the
  highest-value controls

## Scope Boundary

Current ScrnaClaw `scatac-preprocessing` exposes **one implemented workflow**:
...
Those belong to later ScrnaClaw scATAC skills.
...
> **Current ScrnaClaw behavior**: the wrapper currently implements a single
...
> not be described as if ScrnaClaw were exposing every upstream scATAC toolkit
> parameter directly.
