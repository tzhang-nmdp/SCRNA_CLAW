    ...
    param_hints:
      st_pipeline:
        ...
        tips:
          ...
          - "--compute-saturation: enables the upstream saturation curve so the ScrnaClaw report can summarize sequencing depth sufficiency."
...
You are **Spatial Raw Processing**, the ScrnaClaw skill for turning barcoded
...
- **Without it**: users must manually run `st_pipeline`, keep track of IDs / STAR index / GTF paths, convert the output TSV into AnnData, and then remember which object should be passed into downstream ScrnaClaw skills.
...
- **Why ScrnaClaw**: the wrapper standardizes the post-`st_pipeline` object model, report structure, figure-data exports, and downstream routing instead of leaving raw TSV/BED/log files as disconnected artifacts.
...
3. **Run upstream method**: execute `st_pipeline` or use the ScrnaClaw demo path.
...
- "Use my local st_pipeline checkout to build a raw spatial count object and tell me the next ScrnaClaw step."
...
6. **Raw matrix contract**: store raw counts in `X`, `layers["counts"]`, and `raw` so downstream ScrnaClaw preprocessing remains consistent.
...
> **Current ScrnaClaw behavior**: the wrapper intentionally exposes only the first-pass tuning knobs most users need. If a run requires deeper upstream control, the preserved upstream metadata makes it clear what was executed and where to rerun custom `st_pipeline` variants.

## Visualization Contract

ScrnaClaw treats `spatial-raw-processing` visualization as a layered contract:
...
│       └── scrna_claw_stpipeline_run.json
├── figures/
│   ├── *.png
│   └── manifest.json
├── tables/
