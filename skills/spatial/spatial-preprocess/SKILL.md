...
description: >-
  ...
  from spatial-raw-processing), run the current ScrnaClaw scanpy-standard preprocessing workflow,
  ...
  scrna_claw:
    ...
    param_hints:
      scanpy_standard:
        ...
        tips:
          - "--tissue: ScrnaClaw wrapper-level preset that fills QC defaults; reports also record the effective thresholds after preset resolution."
          ...
          - "--n-pcs / --n-neighbors: public Scanpy graph-construction controls; ScrnaClaw reports requested, computed, used, and suggested PCs separately."
...
You are **Spatial Preprocess**, the ScrnaClaw skill for loading raw spatial
...
- **Why ScrnaClaw**: the wrapper preserves raw counts, records the actual effective QC thresholds, keeps platform-specific loading separate from the scientific preprocessing steps, and uses one stable output contract for downstream tools.
...
|--------|-----------|-----------------|---------|
| ScrnaClaw raw spatial counts | `.h5ad` | `raw_counts.h5ad` emitted by `spatial-raw-processing` | `raw_counts.h5ad` |
...
When `--tissue` is set, ScrnaClaw auto-fills default QC thresholds. Explicit
...
9. **Render the standard gallery**: build the ScrnaClaw preprocessing gallery with clustering overview, QC maps, PCA variance guidance, and threshold-context panels.
...
ScrnaClaw treats `spatial-preprocess` visualization as a two-layer system:
...
> **Current ScrnaClaw behavior**: reports include both the user-requested
