...
def convert_st_pipeline_counts_to_adata(
    ...
    *,
    ...
    adata.uns["scrna_claw"] = {
        "skill": "spatial-raw-processing",
        "next_skill": "spatial-preprocess",
        "description": "Raw FASTQ processing via st_pipeline, standardized for downstream ScrnaClaw preprocessing.",
    }
...
def create_demo_upstream_outputs(output_dir: str | Path, *, exp_name: str) -> dict[str, Any]:
    ...
    (upstream_dir / "scrna_claw_stpipeline_run.json").write_text(
        json.dumps(
            {
                "command": ["demo"],
                "runner": "scrna_claw demo",
                "repo_path": None,
                "returncode": 0,
